...
def test_tracked_registry_is_canonical_frozen_and_preassigned() -> None:
    ...
    assert (
        ...
        == "c170ef2188183fd6b184412d2da37de9c12c5d214ebeeba42ea5fccb73567d61"
    )
...
def test_final_campaign_identity_has_public_golden_values() -> None:
    payload = REGISTRY_PATH.read_bytes()
    registry = parse_collection_registry(payload)
    first = registry.assignment("red-battle-v19-01-train")

    assert len(payload) == 6571
    assert (
        registry.registry_sha256
        == "98bef7ce49aa4717a509c72673e77aeb6fd4ea8db99ebf99a099c9e6c05c6388"
    )
    assert (
        registry.execution.source_bundle_sha256
        == "27fcbcd85a6e2f39158b336c62a4df065f13ab04e79519f26011a782d127582b"
    )
    assert (
    ...
    assert (
        ...
        == "c50bf16e5c77f25327b819f0ed856f82f35b1cee26cfb1bf616500e91a2dec5d"
    )
    assert first.assignment_id == "0de5db37503fcbde723ccde309e3c10ba94e8d1512b38fbf9e5a121cbd298722"
...
def test_schedule_expansion_is_deterministic_bounded_and_content_addressed() -> None:
    ...
    run = registry.run("red-battle-v19-01-train")
    ...
    assert first[0].frames == 170
    assert first[-1].battle_plan_id == RED_BATTLE_PLAN_IDS[-1]
    assert first[-1].frames == 145
    assert all(0 <= offset.frames <= 255 for offset in first)
    assert run.schedule_sha256 == "4bef50e3fcf1898a365ffd6363b621748fd911664f263a0b91208f05c70615d7"
...
def test_assignment_ids_are_stable_collision_safe_and_path_free() -> None:
    registry = parse_collection_registry(REGISTRY_PATH.read_bytes())
    first = registry.assignment("red-battle-v19-01-train")
    repeated = registry.assignment("red-battle-v19-01-train")
    ...
    assert metadata["harness_seed"] == 29001
    assert metadata["run_id"] == "red-battle-v19-01-train"
    ...
    first_test = registry.assignment("red-battle-v19-08-test")
...
def test_committed_loader_rejects_a_valid_but_different_registry_digest(
    ...
    repository = _committed_repository(tmp_path)
    changed = _document()
    _runs(changed)[0]["run_id"] = "red-battle-v19-00-train"
...
def test_metadata_names_seed_as_harness_seed_not_a_cartridge_seed() -> None:
    ...
    assert metadata["harness_seed"] == 49001
