...
def test_final_campaign_identity_has_public_golden_values() -> None:
    payload = REGISTRY_PATH.read_bytes()
    registry = parse_collection_registry(payload)
    first = registry.assignment("red-battle-v26-01-train")

    assert len(payload) == 6571
    assert (
        registry.registry_sha256
        == "24d596e678df5a6399bd7b0564d67bcab80f1ac18c99e23570289b9ce40771b4"
    )
    assert (
        registry.execution.source_bundle_sha256
        == "977e585a3c0954ef6921e5ee7d3982cd0e000b9aefda942a91940188331a60a8"
    )
    assert (
    ...
    assert (
        ...
        == "28457c664f6cbdea15c984b68de14802a34b2600f455ef069173af3db69bb767"
    )
    assert first.assignment_id == "9f6f63e29b266efd158b41414ff04eef05feb9d6afef69e99c5ba65b35a29f4b"
...
def test_schedule_expansion_is_deterministic_bounded_and_content_addressed() -> None:
    ...
    run = registry.run("red-battle-v26-01-train")
    ...
    assert first[0].frames == 51
    assert first[-1].battle_plan_id == RED_BATTLE_PLAN_IDS[-1]
    assert first[-1].frames == 233
    assert all(0 <= offset.frames <= 255 for offset in first)
    assert run.schedule_sha256 == "c56b2f27aaad6366279ec707ece1fe3a57f30edca88d087735c451ce32aca9e4"
...
def test_assignment_ids_are_stable_collision_safe_and_path_free() -> None:
    registry = parse_collection_registry(REGISTRY_PATH.read_bytes())
    first = registry.assignment("red-battle-v26-01-train")
    repeated = registry.assignment("red-battle-v26-01-train")
    ...
    assert metadata["run_id"] == "red-battle-v26-01-train"
    ...
    first_test = registry.assignment("red-battle-v26-08-test")
...
def test_committed_loader_rejects_a_valid_but_different_registry_digest(
    ...
    repository = _committed_repository(tmp_path)
    changed = _document()
    _runs(changed)[0]["run_id"] = "red-battle-v26-00-train"
...
def test_metadata_names_seed_as_harness_seed_not_a_cartridge_seed() -> None:
    ...
    assert metadata["harness_seed"] == 56001
