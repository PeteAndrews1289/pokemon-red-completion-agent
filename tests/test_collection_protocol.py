...
def test_final_campaign_identity_has_public_golden_values() -> None:
    payload = REGISTRY_PATH.read_bytes()
    registry = parse_collection_registry(payload)
    first = registry.assignment("red-battle-v80-01-train")

    assert len(payload) == 6671
    assert (
        registry.registry_sha256
        == "fa78f1d3c1c859274b7efe3918ebe0326aaff2788f3aa2bfa4cd23264d830198"
    )
    assert (
        registry.execution.source_bundle_sha256
        == "07b9614f74af305da76482468aeb5ae7b5874c6ba52ce42d157f68ec501fc1c3"
    )
    assert (
    ...
    assert (
        ...
        == "24850882fc2118ac88d418fc29da70f6f84166a8cb792e73b0f04a7d71535f9d"
    )
    assert first.assignment_id == "37a129105694270371fdd1b593bfec6b21437b2a05f3895e4acb06cd0bd8d4d7"
...
def test_schedule_expansion_is_deterministic_bounded_and_content_addressed() -> None:
    ...
    run = registry.run("red-battle-v80-01-train")
    ...
    assert run.schedule_sha256 == "0743778b79e3e23402602fb75eecc58f9c385cae1045ecbb357b17a57e4216a1"
...
def test_assignment_ids_are_stable_collision_safe_and_path_free() -> None:
    registry = parse_collection_registry(REGISTRY_PATH.read_bytes())
    first = registry.assignment("red-battle-v80-01-train")
    repeated = registry.assignment("red-battle-v80-01-train")
    ...
    assert metadata["harness_seed"] == 1080001
    assert metadata["run_id"] == "red-battle-v80-01-train"
    ...
    first_test = registry.assignment("red-battle-v80-08-test")
...
def test_committed_loader_rejects_a_valid_but_different_registry_digest(
    ...
    repository = _committed_repository(tmp_path)
    changed = _document()
    _runs(changed)[0]["run_id"] = "red-battle-v80-00-train"
...
def test_metadata_names_seed_as_harness_seed_not_a_cartridge_seed() -> None:
    assignment = parse_collection_registry(REGISTRY_PATH.read_bytes()).assignment(
        "red-battle-v80-08-test"
    )
    metadata = deepcopy(assignment.metadata_dict())
    serialized = json.dumps(metadata, sort_keys=True)

    assert metadata["harness_seed"] == 1100001
