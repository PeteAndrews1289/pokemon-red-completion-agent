...
def test_final_campaign_identity_has_public_golden_values() -> None:
    payload = REGISTRY_PATH.read_bytes()
    registry = parse_collection_registry(payload)
    first = registry.assignment("red-battle-v88-01-train")

    assert len(payload) == 6671
    assert (
        registry.registry_sha256
        == "7a3d9322323da7c4e22e374199d27813a9d269080d7d4ede1ea8231a0b59ad6b"
    )
    assert (
        registry.execution.source_bundle_sha256
        == "39b5ba6bdc344a6b6b83fbdebe6e766ce37066d837a221464a5b821ced412897"
    )
    assert (
    ...
    assert (
        ...
        == "9cf5c5b0949989469bccbcff6701376efb78eac4bf93f36f33a50550de313794"
    )
    assert first.assignment_id == "a537cf70e806b9d8b0920d26108feaf17ac8895b4fcd300364e8f771a42001b4"
...
def test_schedule_expansion_is_deterministic_bounded_and_content_addressed() -> None:
    ...
    run = registry.run("red-battle-v88-01-train")
    ...
    assert first[0].frames == 161
    assert first[-1].battle_plan_id == RED_BATTLE_PLAN_IDS[-1]
    assert first[-1].frames == 79
    assert all(0 <= offset.frames <= 255 for offset in first)
    assert run.schedule_sha256 == "804c371323f5160f15baf879e8a75415d9257161069b2ceff86d709827435398"
...
def test_assignment_ids_are_stable_collision_safe_and_path_free() -> None:
    registry = parse_collection_registry(REGISTRY_PATH.read_bytes())
    first = registry.assignment("red-battle-v88-01-train")
    repeated = registry.assignment("red-battle-v88-01-train")
    ...
    assert metadata["harness_seed"] == 1380001
    assert metadata["run_id"] == "red-battle-v88-01-train"
    ...
    first_test = registry.assignment("red-battle-v88-08-test")
...
def test_committed_loader_rejects_a_valid_but_different_registry_digest(
    ...
    repository = _committed_repository(tmp_path)
    changed = _document()
    _runs(changed)[0]["run_id"] = "red-battle-v88-00-train"
...
def test_metadata_names_seed_as_harness_seed_not_a_cartridge_seed() -> None:
    assignment = parse_collection_registry(REGISTRY_PATH.read_bytes()).assignment(
        "red-battle-v88-08-test"
    )
    metadata = deepcopy(assignment.metadata_dict())
    serialized = json.dumps(metadata, sort_keys=True)

    assert metadata["harness_seed"] == 1400001
