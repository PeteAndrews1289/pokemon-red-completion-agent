...
def test_final_campaign_identity_has_public_golden_values() -> None:
    payload = REGISTRY_PATH.read_bytes()
    registry = parse_collection_registry(payload)
    first = registry.assignment("red-battle-v78-01-train")

    assert len(payload) == 6659
    assert (
        registry.registry_sha256
        == "f182538ed534ef1316a96edc4829016e106f73144aceb4cac4bdc60dac829264"
    )
    assert (
        registry.execution.source_bundle_sha256
        == "2f7a7c81e708da546f7685bc247afb1bd4b8f9be4bd1a33bf9966d64307bb996"
    )
    assert (
    ...
    assert (
        ...
        == "b4e81bbff4e64deea132c01e8fdf464b59cbf8bd97cd3eb7e2ac0d5687bfcefa"
    )
    assert first.assignment_id == "c0ee69dc13f2e7afc135124e4bd1b178debb1eaaf4fa8f2838c20960179a068b"
...
def test_schedule_expansion_is_deterministic_bounded_and_content_addressed() -> None:
    ...
    run = registry.run("red-battle-v78-01-train")
    ...
    assert run.schedule_sha256 == "42fd699dc1971e74ad6e5e86f8efbde83464e40f716b53eb6bef55cdd6b8cc90"
...
def test_assignment_ids_are_stable_collision_safe_and_path_free() -> None:
    registry = parse_collection_registry(REGISTRY_PATH.read_bytes())
    first = registry.assignment("red-battle-v78-01-train")
    repeated = registry.assignment("red-battle-v78-01-train")
    ...
    assert metadata["harness_seed"] == 970001
    assert metadata["run_id"] == "red-battle-v78-01-train"
    ...
    first_test = registry.assignment("red-battle-v78-08-test")
...
def test_committed_loader_rejects_a_valid_but_different_registry_digest(
    ...
    repository = _committed_repository(tmp_path)
    changed = _document()
    _runs(changed)[0]["run_id"] = "red-battle-v78-00-train"
...
def test_metadata_names_seed_as_harness_seed_not_a_cartridge_seed() -> None:
    assignment = parse_collection_registry(REGISTRY_PATH.read_bytes()).assignment(
        "red-battle-v78-08-test"
    )
    metadata = deepcopy(assignment.metadata_dict())
    serialized = json.dumps(metadata, sort_keys=True)

    assert metadata["harness_seed"] == 990001
