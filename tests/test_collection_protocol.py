...
def test_final_campaign_identity_has_public_golden_values() -> None:
    payload = REGISTRY_PATH.read_bytes()
    registry = parse_collection_registry(payload)
    first = registry.assignment("red-battle-v27-01-train")

    assert len(payload) == 6571
    assert (
        registry.registry_sha256
        == "bfed5ddf6ffaa12b0109e5a7fcbe6649747fad430469e3ceaceba46b5a85d794"
    )
    assert (
        registry.execution.source_bundle_sha256
        == "a45003a6376e5a780731eaadce03209dc8b0f21fbdd31aa1f7956073a8c06811"
    )
    assert (
    ...
    assert (
        ...
        == "07fb4c59cd50c3957a155e7c4003c47990fc8c8617301b38f4a23340d24d988f"
    )
    assert first.assignment_id == "b9dc1a00e3a13b59ce195b20cb89e9940a38aaf752ab580b9c63067166e09e0e"
...
def test_schedule_expansion_is_deterministic_bounded_and_content_addressed() -> None:
    ...
    run = registry.run("red-battle-v27-01-train")
    ...
    assert first[0].frames == 81
    assert first[-1].battle_plan_id == RED_BATTLE_PLAN_IDS[-1]
    assert first[-1].frames == 227
    assert all(0 <= offset.frames <= 255 for offset in first)
    assert run.schedule_sha256 == "81b4ee10a7af9631706bfacd2cc541729ba66679218c96bc302f58c447253712"
...
def test_assignment_ids_are_stable_collision_safe_and_path_free() -> None:
    registry = parse_collection_registry(REGISTRY_PATH.read_bytes())
    first = registry.assignment("red-battle-v27-01-train")
    repeated = registry.assignment("red-battle-v27-01-train")
    ...
    assert metadata["run_id"] == "red-battle-v27-01-train"
    ...
    first_test = registry.assignment("red-battle-v27-08-test")
...
def test_committed_loader_rejects_a_valid_but_different_registry_digest(
    ...
    repository = _committed_repository(tmp_path)
    changed = _document()
    _runs(changed)[0]["run_id"] = "red-battle-v27-00-train"
...
def test_metadata_names_seed_as_harness_seed_not_a_cartridge_seed() -> None:
    ...
    assert metadata["harness_seed"] == 57001
