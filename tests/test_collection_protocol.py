...
def test_tracked_registry_is_canonical_frozen_and_preassigned() -> None:
    ...
    assert (
        ...
        == "ad8ea1623541624fbdff1bb9dd15f7e5d9234b4fd7aa0df0cabf63660878ddf9"
    )
    assert (
        registry.schedule_dry_run.schedule_sha256
        == "db1076f691c753055fb333523b4864b7f5a89cfc172da97737db8ff8372b142d"
    )
...
def test_final_campaign_identity_has_public_golden_values() -> None:
    payload = REGISTRY_PATH.read_bytes()
    registry = parse_collection_registry(payload)
    first = registry.assignment("red-battle-v41-01-train")

    assert len(payload) == 6583
    assert (
        registry.registry_sha256
        == "63f40971fb45be3eaf05dbc1c01146147fdd22875cde570bc97af009d22cac8d"
    )
    assert (
        registry.execution.source_bundle_sha256
        == "0b085eadc591c9bf2c1f87eb0b85ecc5ed276a8bc3c11b0e39f5a1b24a85b010"
    )
    assert (
        registry.execution.behavior_configuration_sha256
        == "6b1ead4078541ca953ed432e90c175710d4c4f7a2b096f14ed9ed5cb6c71b39d"
    )
    assert (
        registry.execution.objective_graph_sha256
        == "453ba1dcecbb33df9e10a911ac93090ff9a5080b07e02a5594e34a015e5bd3b6"
    )
    assert (
        registry.execution.teacher_execution_sha256
        == "a5f2d0426fe5661050474793f469c92df1c9e84554fb4752fd588301eb269183"
    )
    assert first.assignment_id == "98e0f8dee8fd345143a5c373a09124e598238fa061eada1fa120d6802c0ad20b"
...
def test_schedule_expansion_is_deterministic_bounded_and_content_addressed() -> None:
    ...
    run = registry.run("red-battle-v41-01-train")
    ...
    assert first[0].frames == 247
    assert first[-1].battle_plan_id == RED_BATTLE_PLAN_IDS[-1]
    assert first[-1].frames == 177
    assert all(0 <= offset.frames <= 255 for offset in first)
    assert run.schedule_sha256 == "76ce8309d1b93a912847c9877cda8b9b6dc50c1703cf541389dae3013fc1dce9"
...
def test_assignment_ids_are_stable_collision_safe_and_path_free() -> None:
    registry = parse_collection_registry(REGISTRY_PATH.read_bytes())
    first = registry.assignment("red-battle-v41-01-train")
    repeated = registry.assignment("red-battle-v41-01-train")
    ...
    assert metadata["harness_seed"] == 400001
    assert metadata["run_id"] == "red-battle-v41-01-train"
    ...
    first_test = registry.assignment("red-battle-v41-08-test")
...
def test_committed_loader_rejects_a_valid_but_different_registry_digest(
    ...
    repository = _committed_repository(tmp_path)
    changed = _document()
    _runs(changed)[0]["run_id"] = "red-battle-v41-00-train"
...
def test_metadata_names_seed_as_harness_seed_not_a_cartridge_seed() -> None:
    assignment = parse_collection_registry(REGISTRY_PATH.read_bytes()).assignment(
        "red-battle-v41-08-test"
    )
    metadata = deepcopy(assignment.metadata_dict())
    serialized = json.dumps(metadata, sort_keys=True)

    assert metadata["harness_seed"] == 420001
