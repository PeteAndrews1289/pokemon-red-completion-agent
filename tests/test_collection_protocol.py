...
def test_final_campaign_identity_has_public_golden_values() -> None:
    payload = REGISTRY_PATH.read_bytes()
    registry = parse_collection_registry(payload)
    first = registry.assignment("red-battle-v74-01-train")

    assert len(payload) == 6659
    assert (
        registry.registry_sha256
        == "18a14b9c629b594d910317f924373be39e4fc3720aab78422a0482ff37ce20ef"
    )
    assert (
        registry.execution.source_bundle_sha256
        == "6262da8f5937e67ba487a788e84f5bac8714188527c1814003477c3aeccad142"
    )
    assert (
    ...
    assert (
        ...
        == "283e2bb27311bdc0835c9c4b630248475758c8483d3b2bc7a4f205608bab61fc"
    )
    assert first.assignment_id == "7f77261edf8cae4bcbfcd13044a6904b801d4f0c072f1a0f6bfe8d8692e699ef"
...
def test_schedule_expansion_is_deterministic_bounded_and_content_addressed() -> None:
    ...
    run = registry.run("red-battle-v74-01-train")
...
def test_assignment_ids_are_stable_collision_safe_and_path_free() -> None:
    registry = parse_collection_registry(REGISTRY_PATH.read_bytes())
    first = registry.assignment("red-battle-v74-01-train")
    repeated = registry.assignment("red-battle-v74-01-train")
    ...
    assert metadata["run_id"] == "red-battle-v74-01-train"
    ...
    first_test = registry.assignment("red-battle-v74-08-test")
...
def test_committed_loader_rejects_a_valid_but_different_registry_digest(
    ...
    repository = _committed_repository(tmp_path)
    changed = _document()
    _runs(changed)[0]["run_id"] = "red-battle-v74-00-train"
