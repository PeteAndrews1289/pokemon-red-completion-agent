...
def test_final_campaign_identity_has_public_golden_values() -> None:
    payload = REGISTRY_PATH.read_bytes()
    registry = parse_collection_registry(payload)
    first = registry.assignment("red-battle-v53-01-train")

    assert len(payload) == 6659
    assert (
        registry.registry_sha256
        == "b897b4ab9481dbdd268639a745e8e64315ab521e5156580ab9bdee273fb34f03"
    )
    assert (
        registry.execution.source_bundle_sha256
        == "1fe1bd295d9ce404c3b9996572e66ea1594edc1872c9a047f65b0b97c461557b"
    )
    assert (
    ...
    assert (
        ...
        == "9dd4519e0e73e905e5ed17ee3090ae436b99df9b2fcd5706c25f6107f670d3e6"
    )
    assert first.assignment_id == "a985245e2d21ce100caee30b924324fd3579ecca885625239db53f8df89c3c7c"
...
def test_schedule_expansion_is_deterministic_bounded_and_content_addressed() -> None:
    ...
    run = registry.run("red-battle-v53-01-train")
    ...
    assert first[0].frames == 244
    assert first[-1].battle_plan_id == RED_BATTLE_PLAN_IDS[-1]
    assert first[-1].frames == 8
    assert all(0 <= offset.frames <= 255 for offset in first)
    assert run.schedule_sha256 == "84e50cd7b701c902dc4c3f9066275904ff78abc4e4e6bdc0b71d07210621b06a"
...
def test_assignment_ids_are_stable_collision_safe_and_path_free() -> None:
    registry = parse_collection_registry(REGISTRY_PATH.read_bytes())
    first = registry.assignment("red-battle-v53-01-train")
    repeated = registry.assignment("red-battle-v53-01-train")
    ...
    assert metadata["harness_seed"] == 610001
    assert metadata["run_id"] == "red-battle-v53-01-train"
    ...
    first_test = registry.assignment("red-battle-v53-08-test")
...
def test_committed_loader_rejects_a_valid_but_different_registry_digest(
    ...
    repository = _committed_repository(tmp_path)
    changed = _document()
    _runs(changed)[0]["run_id"] = "red-battle-v53-00-train"
...
def test_metadata_names_seed_as_harness_seed_not_a_cartridge_seed() -> None:
    assignment = parse_collection_registry(REGISTRY_PATH.read_bytes()).assignment(
        "red-battle-v53-08-test"
    )
    metadata = deepcopy(assignment.metadata_dict())
    serialized = json.dumps(metadata, sort_keys=True)

    assert metadata["harness_seed"] == 630001
