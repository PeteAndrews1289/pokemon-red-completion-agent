...
def test_final_campaign_identity_has_public_golden_values() -> None:
    payload = REGISTRY_PATH.read_bytes()
    registry = parse_collection_registry(payload)
    first = registry.assignment("red-battle-v28-01-train")

    assert len(payload) == 6571
    assert (
        registry.registry_sha256
        == "8c273ea2fe12931bdad6ae7a35c0136df28c418aa2274aba503c23d3cc130a4b"
    )
    assert (
        registry.execution.source_bundle_sha256
        == "7ec106424065dbbf6d0981cbb54f805c9725d0e413f1178413e64df10c1b73b7"
    )
    assert (
    ...
    assert (
        ...
        == "47a8864ca3f5d9401db80b6c501a0e3277551b58bf35ac805424aed7afa9b51e"
    )
    assert first.assignment_id == "d9035ad9e8945235dad260e0843b78a0208e621640a8ab3edd76bf84738f2860"
...
def test_schedule_expansion_is_deterministic_bounded_and_content_addressed() -> None:
    ...
    run = registry.run("red-battle-v28-01-train")
    ...
    assert first[0].frames == 78
    assert first[-1].battle_plan_id == RED_BATTLE_PLAN_IDS[-1]
    assert first[-1].frames == 6
    assert all(0 <= offset.frames <= 255 for offset in first)
    assert run.schedule_sha256 == "afc24d58ddc8a3e91ed6fe55e5d4f30b7ef34e8977d04b638fed738bba87dd6b"
...
def test_assignment_ids_are_stable_collision_safe_and_path_free() -> None:
    registry = parse_collection_registry(REGISTRY_PATH.read_bytes())
    first = registry.assignment("red-battle-v28-01-train")
    repeated = registry.assignment("red-battle-v28-01-train")
    ...
    assert metadata["harness_seed"] == 38001
    assert metadata["run_id"] == "red-battle-v28-01-train"
    ...
    first_test = registry.assignment("red-battle-v28-08-test")
...
def test_committed_loader_rejects_a_valid_but_different_registry_digest(
    ...
    repository = _committed_repository(tmp_path)
    changed = _document()
    _runs(changed)[0]["run_id"] = "red-battle-v28-00-train"
...
def test_metadata_names_seed_as_harness_seed_not_a_cartridge_seed() -> None:
    assignment = parse_collection_registry(REGISTRY_PATH.read_bytes()).assignment(
        "red-battle-v28-08-test"
    )
    metadata = deepcopy(assignment.metadata_dict())
    serialized = json.dumps(metadata, sort_keys=True)

    assert metadata["harness_seed"] == 58001
