...
def test_final_campaign_identity_has_public_golden_values() -> None:
    payload = REGISTRY_PATH.read_bytes()
    registry = parse_collection_registry(payload)
    first = registry.assignment("red-battle-v79-01-train")

    assert len(payload) == 6671
    assert (
        registry.registry_sha256
        == "e97032237e9b3a30bbca538312041a813f6630ddb569bdab711e07439ebd1adb"
    )
    assert (
        registry.execution.source_bundle_sha256
        == "914a15ccf4ec82619454c86a88c39d985a16c9e956be7ae786f3d1a3873165b6"
    )
    assert (
    ...
    assert (
        ...
        == "08ffd4587daff071de6767475025c5ea07037bea4186960c4b3f566a9d144f22"
    )
    assert first.assignment_id == "60962900ea20f6c9c0d6c6dc3c522700c3d8ad29d88492ebc84065dd679d3f59"
...
def test_schedule_expansion_is_deterministic_bounded_and_content_addressed() -> None:
    ...
    run = registry.run("red-battle-v79-01-train")
    ...
    assert first[-1].frames == 240
    assert all(0 <= offset.frames <= 255 for offset in first)
    assert run.schedule_sha256 == "5dd99203f3e4037f9b88eb55754c0a7365034c902528f42ad61fcd69622698f1"
...
def test_assignment_ids_are_stable_collision_safe_and_path_free() -> None:
    registry = parse_collection_registry(REGISTRY_PATH.read_bytes())
    first = registry.assignment("red-battle-v79-01-train")
    repeated = registry.assignment("red-battle-v79-01-train")
    ...
    assert metadata["harness_seed"] == 1030001
    assert metadata["run_id"] == "red-battle-v79-01-train"
    ...
    first_test = registry.assignment("red-battle-v79-08-test")
...
def test_committed_loader_rejects_a_valid_but_different_registry_digest(
    ...
    repository = _committed_repository(tmp_path)
    changed = _document()
    _runs(changed)[0]["run_id"] = "red-battle-v79-00-train"
...
def test_metadata_names_seed_as_harness_seed_not_a_cartridge_seed() -> None:
    assignment = parse_collection_registry(REGISTRY_PATH.read_bytes()).assignment(
        "red-battle-v79-08-test"
    )
    metadata = deepcopy(assignment.metadata_dict())
    serialized = json.dumps(metadata, sort_keys=True)

    assert metadata["harness_seed"] == 1050001
