...
def test_final_campaign_identity_has_public_golden_values() -> None:
    payload = REGISTRY_PATH.read_bytes()
    registry = parse_collection_registry(payload)
    first = registry.assignment("red-battle-v75-01-train")

    assert len(payload) == 6659
    assert (
        registry.registry_sha256
        == "f346967186256cb371c652544f48773ebcd425455cea9632ecf5b68a6cd84023"
    )
    assert (
        registry.execution.source_bundle_sha256
        == "2e43fc6160eee0ce6e2a9ad430ea96785c2dd60141b38525ca3547698d4d0a3f"
    )
    assert (
    ...
    assert (
        ...
        == "8a378e3d60705541219eb4f86f7257d1d7368042ab00dcc9b45bb7f51b2531cc"
    )
    assert first.assignment_id == "71b9309b555037b7b4ebbddcf6e2de0c5df96526928e2b17f4a9a73f37285c5a"
...
def test_schedule_expansion_is_deterministic_bounded_and_content_addressed() -> None:
    ...
    run = registry.run("red-battle-v75-01-train")
...
def test_assignment_ids_are_stable_collision_safe_and_path_free() -> None:
    registry = parse_collection_registry(REGISTRY_PATH.read_bytes())
    first = registry.assignment("red-battle-v75-01-train")
    repeated = registry.assignment("red-battle-v75-01-train")
    ...
    assert metadata["run_id"] == "red-battle-v75-01-train"
    ...
    first_test = registry.assignment("red-battle-v75-08-test")
...
def test_committed_loader_rejects_a_valid_but_different_registry_digest(
    ...
    repository = _committed_repository(tmp_path)
    changed = _document()
    _runs(changed)[0]["run_id"] = "red-battle-v75-00-train"
