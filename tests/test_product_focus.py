...
def test_tracked_focus_is_canonical_and_reports_evidence_backed_learning_progress() -> None:
    ...
    assert state.progress["causal_train_examples"] == 8
...
def test_checker_binds_discovery_docs_and_pull_request_mission_check() -> None:
    ...
    assert rows == ("Causal Train Example · train: 8/60",)
...
def test_learning_lane_accepts_evidence_backed_causal_train_examples() -> None:
    ...
    assert focus_scorecard(state) == (("Causal Train Example · train", 8, 1),)
...
def test_focus_dashboard_is_view_only_and_does_not_overclaim_training() -> None:
    ...
    assert public["stage_progress"] == pytest.approx(8 / 60)
    assert public["actions"] == 0
    assert "Clustered Red curriculum" in public["stage"]
    assert public["experiment"]["zero_shot"] == {"completed": 8, "total": 60}  # type: ignore[index]
    ...
    assert "Causal Train Example 8/60" in encoded
