...
def test_tracked_focus_is_canonical_and_reports_evidence_backed_learning_progress() -> None:
    ...
    assert len(state.retired_lanes) == 55
...
def test_focus_dashboard_is_view_only_and_does_not_overclaim_training() -> None:
    ...
    assert "Red multi-family living-Dex option-value curriculum V3" in public["stage"]
    ...
    assert "strict publication gate passed" in encoded
    assert "action-free freeze" in encoded
    assert "durable 8+8 plan" in encoded
