...
def test_tracked_focus_is_canonical_generated_and_bound_to_learning_evidence() -> None:
    ...
    assert focus_progress_fraction(state) == 0.078125
...
def test_focus_dashboard_is_view_only_and_does_not_overclaim_training() -> None:
    ...
    assert public["stage_progress"] == 0.078125
    assert public["actions"] == 0
    assert public["experiment"]["zero_shot"] == {"completed": 7, "total": 48}  # type: ignore[index]
    ...
    assert "Nine of 53 candidate trials" in encoded
