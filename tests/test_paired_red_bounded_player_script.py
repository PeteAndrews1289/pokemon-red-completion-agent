...
@pytest.mark.parametrize('field', ['cut', 'surf'])
def test_capture_cut_is_an_ordered_prospective_transition(monkeypatch, field):
    from test_red_living_dex_wild_corridor import _local_discovery_profile
    module = runpy.run_path(str(SCRIPT))
    derive = module['_regional_profiles']
    monkeypatch.setitem(derive.__globals__, '_route_world', lambda _: object())
    before = _local_discovery_profile()
    old, new = derive(before, ('capture-fly', 'capture-' + field), object())
    assert field + '_transport' not in old.providers[0].parameters
    assert new.providers[0].parameters[field + '_transport'] is True
