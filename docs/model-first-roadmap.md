...
## Current checkpoint: successor readiness passed; collect one causal outcome now

PR 106 published the exact-plan-bound 16+4 successor consumer. The first exact-main run exposed a
test-only assumption about squash history; PR 107 repaired that assertion without changing the
executable bundle. Exact main `248346b0` and push CI `33290594694/1` are green. One authenticated
ROM-free preflight then reopened successor train ordinal 0 and its selected root exactly once with
claims, actions, frames, commitments, outcomes, predictions, teachers, fits, and development
access all zero. A preceding disposable-runtime bootstrap miss rejected generated bytecode before
source or private input and is recorded in the path-free receipt.

The next product-aligned sequence is:

1. publish the path-free preflight receipt and require green exact-main CI;
2. execute successor train ordinal 0 exactly once through the durable selected-arm journal;
...
4. stop and reorient before ordinal 1 or any model fit;
5. continue one-shot train rows only after each reorientation until the authentic corpus reaches
   at least eight, then run the explicitly non-authoritative integration fit;
...
The preflight exhausted the lane's one-session infrastructure allowance. Do not add another
wrapper, preflight, census, teacher route, full replay, development access, sealed Red run, or
Crystal execution. **Causal train 6/60 · powered fit 0 · authority 0 · transfer 0.** See the
