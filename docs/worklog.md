...
> supported candidate feature rows** and **7 varying target heads**, produced **225/225 finite
> coefficients**, and reopened the immutable artifact as identical bytes and an identical typed
> model. It made zero gameplay predictions, development reads, teacher queries, controller
> actions, emulator frames, authority promotions, or Crystal accesses.
>
> One earlier command launch selected an initialized but unrelated private store and failed closed
> at corpus authentication with **fit executions 0 · claims 0**. It was not a learner retry. The
> board is now **causal train 8/60 · integration fit 1/1 · powered fit 0 · authority 0 · transfer
> 0**. This proves the honest learning path works end to end; it does not show that an eight-row
> model can play. Next publish the
> [path-free fit result](../docs/evidence/red-living-dex-causal-integration-fit-result-v1-2026-08-30.json),
> require green exact-main CI, then attempt untouched successor train ordinals **2–7** once each as
> a fixed six-card R1 throughput pilot. Preserve every failure, keep development sealed, and stop
> after the batch to measure yield, support, cost, and examples per wall-clock hour before freezing
> the powered clustered design.
...
> 36 train and 23 development; 55 support at least one genuine menu**. The default scheduler still
> forms eight fresh train lineages plus four development lineages with all seven Red-supported
> portable kinds, one lineage per row, and zero overlap. The census performed zero actions,
> frames, claims, outcomes, predictions, or fits.
>
> The fixed successor is deliberately larger than the minimum repair: **16 distinct train
> lineages plus 4 read-only development lineages**, maximum one scenario per lineage. At the
> observed 5/8 setup yield it would produce ten settled examples in expectation, without treating
> that rate as a guarantee or selecting roots after rehearsal. The sealed-plan validator and
> train boundary have now been generalized and locally qualified so schedule ordinal 15 is legal
> while Red template ordinals remain cartridge-bound to 0–14. The successor has a new private
> namespace and runner identity; it cannot retry or overwrite the consumed pilot. The full suite
> passed 5,777 tests and the focused boundary suite passed 49, with zero protected effects.
> **Causal train 6/60 · powered fit 0 · authority 0 · transfer 0.** See the
> [capacity result](../docs/evidence/red-living-dex-clustered-successor-capacity-v1-2026-08-29.json)
> and [local qualification](../docs/evidence/red-living-dex-clustered-successor-freezer-local-qualification-v1-2026-08-29.json).
...
> exposes only train ordinals 0–7, keeps schedule and Red-template ordinals distinct, preserves the
> upstream episode lineage, atomically protects logical and physical roots, commits one random arm
> before controller input, and records only the selected arm's independently observed outcome.
> Development, teacher labels, counterfactual targets, sealed Red, and Crystal remain structurally
> absent.
>
> Its ROM-free preflight then authenticated one exact available train pair with **root reads 1 ·
> claims/actions/frames/commitments/outcomes/predictions/fits 0**. An initial process launch failed
> at the interpreter boundary because the minimal locale enabled Python UTF-8 mode; it reached no
> source, runtime, private root, or protected effect. The corrected authenticated preflight ran once
> and passed. No authentic scenario ran, so **causal train 1/60 · powered fit 0 · authority 0 ·
> transfer 0**. Next: publish the [path-free result](../docs/evidence/red-living-dex-clustered-train-preflight-result-v1-2026-08-29.json),
> require exact-main green CI, execute exactly one selected-arm train outcome, and reorient before
> another row or any model fit.
...
> records **4 DEVELOPMENT RECORDS PRIVATELY DECODED DURING INVENTORY · ROW CONTENTS PUBLISHED 0 ·
> ROOTLESS FIT 1 COUNTED BUT EVALUATION-INELIGIBLE · COMPARISON IDENTITY UNCONSUMED AND INELIGIBLE
> · COMPARISON EXECUTIONS/RESULTS 0 · COUNTER DELTAS 0**.
>
> The comparison never ran, so this is not a failed comparison and supplies no unseen metric. The
> fit happened and remains counted, but it cannot support a held-out claim; no evidence shows that
> its optimizer used development labels or that a stored fit record mismatched. Active maintenance
> `rootless-living-dex-dependency-evaluation-integrity-qualification-v1` is public/synthetic only:
> implement a metadata-only commitment inspector and a strict loaded-fit-to-bundle semantic join,
> then stop. It may not open private inputs, reuse the old fit or four decoded openings, fit,
> compare, play Red or Crystal, promote authority, or claim completion or living-Pokédex transfer.
> Boards remain **30/15/4/3/0/0**, development **14/4/0/1/1**, and rootless **8/8/1/0**.
...
>
> The immediately following V1 campaign, fit, and comparison-preflight checkpoints are preserved as
> history. Their claims that the four development openings remained sealed or that disclosure was
> zero were later invalidated by this audit.

> **Previous product focus (August 20, 2026; later invalidated): the comparison preflight was
> recorded as qualified and unopened.** Exact execution source
...
> recorded **FIT-BOUND PREFLIGHT PASSED · COMMITMENTS 4 · COMPARISON IDENTITY UNUSED ·
> DEVELOPMENT PAYLOADS OPENED 0** at the time. The integrity receipt above supersedes its
> zero-disclosure and evaluation-readiness claims.
>
> The comparison authorization described here is retired without execution or retry. Its identity
> remains unconsumed but is ineligible; no aggregate or row-level result exists.

> **Previous product focus (August 20, 2026; sealing claim later invalidated): the first rootless
> dependency fit completed.** Exact execution source
...
> records **8 train rows · accuracy 1.0 · cross-entropy 0.693147 → 0.205636**. Its
> development-opened-zero field is a historical claim superseded by the integrity receipt above.
>
...
> `rootless-living-dex-dependency-comparison-qualification-v1` was the historical next lane. It and
> the fit are now retired without reuse or retry. This remains an abstract dependency learner—not
> a model proven to play or complete Pokémon.

> **Previous product focus (August 20, 2026; sealing claim later invalidated): eight rootless
> outcomes were admitted; fit once.**
...
> consumed**. It reported **4 development openings still sealed**, a claim later invalidated by the
> integrity audit. It adds exactly **8 synthetic rootless train
...
>
> The historical next step was one fixed train-only fit. That fit later completed and remains
> counted; the sealing claim and subsequent comparison path are superseded by the integrity audit.

> **Previous product focus (August 20, 2026; zero-disclosure claim later invalidated): the rootless
> campaign was training-ready.** Exact
...
> The freeze and preflight made zero predictions or transitions and moved no learning counter. Its
> zero-development-disclosure claim was later invalidated because inventory decoded all four
> records.
>
> The historical next step was the fixed eight-row train campaign. It later completed, but its
> zero-disclosure claim is not valid evidence that development stayed unopened.
