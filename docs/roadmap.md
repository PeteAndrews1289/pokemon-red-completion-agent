...
> [model-first roadmap](model-first-roadmap.md). The current endpoint is Model115 with84 registrations
> and115 settled examples. Diagnose the retained income-verification mismatch before the next frozen
> choice. Full fresh-run Red story and local Pokédex completion must precede any ROM hack.
...
> **September 3 Red bridge:** The existing Red state adapter, executable goal enumerator and living
> collection projector now feed the generic seam through one thin module. Its semantic digest binds
> public Red progress, the exact title-neutral policy input and the collection checkpoint while
> omitting private bindings. The combined gate is **13/13 ROM-free tests** with zero gameplay.
...
> controller actions · 146,341 emulator frames**. Teachers, counterfactual and unselected targets,
> development, predictions, fits, retries, and private publication were zero.
>
> Next publish the [path-free terminal](evidence/red-living-dex-clustered-successor-train-ordinal-1-result-v1-2026-08-30.json),
> require green exact-main CI, and run one action-free train-only integration-readiness audit over
> the complete eight-example denominator. Do not retry ordinals 0 or 1, drop awkward rows, open
> development, fit before the audit, run sealed Red or Crystal, or replay the game. **Causal train
> 8/60 · powered fit 0 · authority 0 · transfer 0.**
