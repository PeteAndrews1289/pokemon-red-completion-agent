...
> registrations. A failed model-selected fishing route became row112; the exact state then
> recovered without a training label, and model112 selected restore from a rebuilt five-choice
> menu. That choice remains unexecuted. A deterministic teacher has completed Red, but the learned
...
| Current retained Red state | **Partial** | 83 registered species, 63 living species and 67 specimens reopen at an authenticated input-ready model112 checkpoint. |
...
with zero learning credit. Model112 then selected restore from the rebuilt menu; execution is next.
...
- The exact failed state recovered input-ready control in 8 actions and 576 frames with zero
  teacher labels. Model112 then selected restore from a five-way menu; that choice is unexecuted.
...
| Latest recovery and menu | 8 actions; 576 frames; 0 labels; 83 registrations retained; model112 selected restore with probability 0.599149 | Durable continuation works; the selected restore has not executed and is not a new training row. |
...
- Capture preparation searches all twelve verified boxes and preserves the complete
  ...
  stopped at dialogue. The retained checkpoint recovered and model112 selected restore, but that
  exact choice has not executed.
...
[model112 recovery and menu session](work-sessions/2026-09-13-model112-dialogue-recovery-and-menu.md).
Historical receipts
