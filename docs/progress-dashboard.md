...
>
> **Current rail:** Show **PR 106 → CONSUMER PUBLISHED → SQUASH-HISTORY TEST FAILURE → ONE
> TEST-ONLY REPAIR → PR 107 → MAIN 248346B0 → CI 33290594694/1 GREEN → DISPOSABLE RUNTIME CACHE
> REJECTED BEFORE SOURCE/PRIVATE ACCESS → CACHE-FREE CLOSURE 3DD20373 ✓ → AUTHENTICATED ROM-FREE
> PREFLIGHT → SUCCESSOR TRAIN 0 → ROOT READS 1 → CLAIMS/ACTIONS/FRAMES/COMMITMENTS/OUTCOMES/
> PREDICTIONS/TEACHERS/FITS 0**. Continue **PATH-FREE RESULT → EXACT-MAIN CI → ORDINAL 0 EXACTLY
> ONCE → DURABLE SELECTED-ARM TERMINAL → REORIENT → 8+ AUTHENTIC EXAMPLES → NON-AUTHORITATIVE FIT
> → 60 → UNTOUCHED RED → FROZEN SUPPORTED REPRESENTATION → CRYSTAL → CROSS-GAME LIVING POKEDEX**.
> Keep **CAUSAL TRAIN 6/60 · POWERED FIT 0 · AUTHORITY 0 · TRANSFER 0** visible. Cross out
> **ANOTHER PREFLIGHT · WRAPPER · CENSUS · RETRY · SECOND ROW BEFORE REORIENTATION · DEVELOPMENT
> · TEACHER TARGET · FULL REPLAY · CRYSTAL NOW**. Link the
> [preflight result](evidence/red-living-dex-clustered-successor-train-preflight-result-v1-2026-08-29.json).
>
