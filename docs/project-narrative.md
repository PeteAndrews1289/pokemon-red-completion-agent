...
| The first pre-ship training rehearsal entered Diglett's Cave but immediately crossed its entrance warp back to the gate. | The linked warp briefly exposed `(4, 4)` and `(37, 31)` before settling at the usable cave coordinate; the training loop began holding a direction during that transition. | Training waits for two consecutive input-ready observations on a non-warp cave tile before seeking an encounter. The excluded-warp guard remains unchanged and the rehearsal remains uncounted. |
