...
| The first pre-ship training rehearsal entered Diglett's Cave but immediately crossed its entrance warp back to the gate; the first readiness repair then waited forever at `(37, 31)`. | The linked warp briefly exposed `(4, 4)`, but the next rehearsal proved `(37, 31)` is the stable arrival tile rather than another transitional coordinate. | Training waits for two consecutive input-ready observations after the `(4, 4)` handoff, accepts the stable `(37, 31)` arrival, and still guards the two-tile encounter corridor from the exit. Both rehearsals remain uncounted. |
