...
| The first pre-ship training rehearsal entered Diglett's Cave but immediately crossed its entrance warp back to the gate; the first readiness repair then waited forever at `(37, 31)`; accepting that tile exposed one further return-warp boundary. | The linked warp briefly exposes `(4, 4)`, then settles at `(37, 31)`. That coordinate is a safe arrival, but stepping back onto it after moving north activates the return warp. | Training waits for two input-ready observations, makes one verified northbound step to the safe `(37, 30)` anchor, and bounces only between `(37, 30)` and `(37, 29)`. The exit tile is used again only after training stops. All rehearsals remain uncounted. |
