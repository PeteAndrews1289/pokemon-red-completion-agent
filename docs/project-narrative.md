...
| The Bide-funded replay bought the correct Snorlax reserve, caught Snorlax in five throws, completed Safari, Koga, Erika, and reached Saffron at checkpoint 239 before Silph capacity cleanup rejected the full twenty-slot bag. | The cleanup archived exactly three obsolete route items and therefore accepted at most nineteen input slots. This schedule legitimately retained one additional recovery stack, so three deposits would leave seventeen slots instead of the required sixteen. | Capacity cleanup now selects its deposit set from live occupancy: the original three items remain mandatory, and a twenty-slot lineage additionally archives the now-obsolete Silph Scope. Both paths must finish at sixteen or fewer slots, while missing route items or any larger unsupported state still fail closed. The qualification remains private and uncounted. |
...
repeated misses. That v3 failure remains in its denominator and v3 cannot be fitted. V4 now binds
the repaired bridge recovery, uses the exposed v3 seed only for its uncounted rehearsal, and has
twelve fresh counted slots. The v4 campaign remains unopened pending that complete replay. No
