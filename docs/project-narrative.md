...
## August 30: the key fits; the next turn must create a lesson

The successor finally crossed its readiness boundary. The exact 16+4 consumer was published, and
GitHub immediately found a provenance-test assumption that local testing had missed: a squash
merge does not preserve the feature commit as a reachable ancestor. The executable code was fine.
One bounded test-only repair replaced ancestry with immutable component hashes, and exact main
passed every CI gate without changing the executable bundle.

The first production launch then demonstrated why the runtime boundary exists. A copied virtual
environment contained generated bytecode, so the process stopped before it authenticated source
or touched a private capture. An inert diagnostic found that public cache contamination; a fresh
cache-free copy reproduced the reviewed dependency closure. The authenticated preflight then read
successor training root zero once and stopped with no ROM, claim, button, frame, randomized arm,
outcome, prediction, teacher, or fit.

That is the end of infrastructure permission, not a learning milestone. The scoreboard remains
six causal examples out of sixty. The next operation must execute the frozen ordinal-zero lesson
exactly once. It will commit one random collection-oriented option before input and retain only
that option's independently observed consequence. If setup fails, the empty desk remains empty;
there is no retry or replacement. This is the project's recurring theme in miniature: trustworthy
autonomy grows from measured decisions, not from quietly editing the curriculum until it works.
