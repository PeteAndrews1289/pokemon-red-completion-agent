...
| The evidence-gated replay passed all eleven Lavender trainer lessons, seven safe wild escapes, the complete party and healing contract, and every route gate, but arrived with zero Awakenings after two legitimate uses. | The reserve was still sized for a one-use lineage. The exact diagnostic showed ₽1,761 remained after eighteen Super Potions and all other restocks, so the missing contingency was affordable rather than an economy conflict. | Vermilion now buys two Awakenings, producing a three-item total reserve with the carried copy. The contract still permits at most two uses and requires at least one remaining for Pokémon Tower; the additional ₽200 is propagated through the exact purchase ledger instead of relaxing the downstream requirement. |
