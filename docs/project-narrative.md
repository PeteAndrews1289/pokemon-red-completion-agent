...
| The left-side Center detour then reached the same boundary but found the wandering NPC parked immediately to the player's left. | The NPC can occupy different adjacent tiles across otherwise valid held-out rehearsals, so choosing an arbitrary side was still timing-dependent. | The exit detour now uses the proven open PC-side corridor to the right, descends below the NPC's walking row, and rejoins the same doorway. The exact source must still requalify end to end before collection. |
