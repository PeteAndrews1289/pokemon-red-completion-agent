...
[Audit and next bounded plan](../audits/red-story-entry-qualification-2026-09-08.md).
...
deterministic teacher exists only to create genuinely new episode lineages and independently
...
5. materialize only its train roots, retain every terminal disposition, and rerun the action-free
   maximum-matching census; and
...
## Previous checkpoint: generate fresh independent Red train capacity
...
records 24 direct, 177 combined related-regression, and 5,111 repository tests. No ROM, private
capture, controller input, frame, behavior draw, learner label, outcome, prediction, teacher query,
fit, authority, or transfer result was used.

The typed producer now accepts coherent Red/traversal observations, available executable offers
from the exact published provider allowlist, and complete semantic-router plans. It derives the
canonical fifteen slots, forty-five provider arms, three local options, and forty-two routed
options without invoking an executor or verifier. The owner-private freezer rereads and rehashes
the complete catalog on all seventeen source interactions, shares the materializer's independent
protected-effect meter, and publishes only aggregate path-free evidence. Synthetic witnesses
qualify the implementation but are not authentic Red evidence.

The next gate must:

1. under a separately recorded action-free gate, bind approved Red captures, route terminals,
   fresh observations, and existing semantic providers to the published source protocol;
2. authenticate the complete protected input set
   and freeze exactly one canonical 10+5 private plan;
3. preserve all forty-five genuine arms, fourteen routed joins, and one local slot;
4. prove every controller, frame, learner, model, claim, and teacher delta remains zero; and
5. publish only aggregate success or finite failure evidence, then stop before invoking the setup
   runner.
...
## Previous checkpoint: preflight one authentic dual-capability Red scenario
...
the
[integrity-failure result](../evidence/rootless-living-dex-dependency-comparison-integrity-failure-v1-2026-08-20.json)
(SHA-256 `0eee3874c664bdda0bdd6f4cfb3ec3f3e6b36f8b442173a37f8dcdd5a6cf0686`)
shows that common inventory privately decoded all four development records before the comparison
boundary. No row content was published. It also found that the loaded fit was not strictly joined
to all execution-manifest semantic pins.

The comparison never ran. Its identity remains unconsumed but is ineligible, and the old fit and
four openings retire without reuse or retry. The fit still counts as one completed synthetic
parameter update, but it is evaluation-ineligible; do not infer that the optimizer used
development labels or that a stored fit record actually mismatched. Boards remain
**30/15/4/3/0/0**, development **14/4/0/1/1**, and rootless **8/8/1/0**.

Active maintenance `rootless-living-dex-dependency-evaluation-integrity-qualification-v1` is one
public/synthetic falsifier: implement a metadata-only commitment inspector and enforce an exact
loaded-fit-to-record/manifest/terminal/model/dataset semantic join, mutation-test both boundaries,
and stop. No private input, old artifact reuse, fit, comparison, gameplay, authority, completion,
Crystal, or transfer belongs in this lane. A pass may only support a separately frozen fresh
experiment; it is not learned competence.
...
## Previous checkpoint: comparison preflight was recorded; later invalidated
...
recorded an unused comparison identity and zero development-payload disclosure or counter delta at
the time. The newer integrity receipt supersedes its disclosure and readiness claims.

The planned comparison never ran. Its old lane and unconsumed identity are retired without retry;
the four decoded records may not be reused as unseen development.

## Previous checkpoint: the dependency ranker fit; sealing claim later invalidated
...
The scoped rootless fit board advanced to 1/1; the comparison remained zero. The later audit
invalidated the development-disclosure-zero and held-out-readiness claims while preserving the fit
count.

The historical comparison-qualification instruction is superseded. The old fit, openings, and
comparison identity may not be reopened, reused, or retried.

## Previous checkpoint: the fixed train set was admitted; sealing claim later invalidated
...
Its claim that the four development openings remained sealed was later invalidated. Rootless
outcomes and atomic episodes are 8/8;
scoped fits and comparisons are 0/1. Legacy boards remain 30/15 outcomes, 4 fits, 3 comparisons.

The historical next step was the one-shot interaction-ranker fit. It later completed and remains
counted, but its sealing and comparison-readiness claims are superseded.

## Previous checkpoint: campaign frozen; zero-disclosure claim later invalidated
...
namespaces unused. Predictions, transitions, outcomes, fits, gameplay, and every counter delta were
zero. Its development-disclosure-zero claim was later invalidated because inventory decoded all
four records.

The historical next step was the bounded eight-row train campaign. It later completed, but its
zero-disclosure claim was not a valid basis for held-out evaluation.
...
external source, runtime, model, context plan, NumPy, and PyBoy before any protected read. The
contract fixes exactly four authenticated nonsealed train roots and twelve trials, three per root,
behind a zero-action campaign freeze/preflight. A durable global claim must precede prediction.
Hard action/frame limits and strict propensity, identity, and living-collection admission fail
closed. Evolution is excluded pending lineage evidence.

The candidate remains unpublished and unqualified. Campaign freezes, global claims, predictions,
controller actions, frames, gameplay, outcomes, episodes, and fits remain zero. The legacy board is
still **30 train · 15 development · 3 fits · 3 comparisons · authority 0 · transfer 0**, and all
five product-focus-v2 counters remain zero. Publish the candidate, require green CI on the exact
published head, then freeze the exact campaign and run one zero-action preflight. That gate is not
training readiness, gameplay evidence, a fit, or new authority.

## Previous checkpoint: V4 failed safely; qualify repeatable model-led development

Published source `20d4b1532ee78a3ffc5b762b2f90ae536dfa2022` passed CI run `32092299544`,
attempt 1. Its single V4 preflight emitted the sanitized `readiness_authentication` stage. That is
not a diagnosis. The invocation wrote no execution claim, authorized no execution identity, made
no goal-manager decision or prediction, sent no controller input, advanced no frame, and produced
no gameplay, outcome, or episode. Project policy durably closed the exact root account-wide; it
may never retry. See the
...
candidate implementation is under independent review, unpublished, and unqualified. The
[frozen design](../evidence/repeatable-goal-manager-development-design-v1-2026-08-17.json) specifies
four authenticated nonsealed train roots with three trials each, spanning story advancement, team
recovery, storage management, and species acquisition. A declared 15% exploration mixture samples
the learned policy and records exact behavior propensities. Verified success scores +1, verified
failure scores -1, and interrupted choices remain durable but create no update target. Hard action
and frame bounds, durable decisions and terminals, and zero teacher/fallback are mandatory.

Review and publication precede any campaign freeze or zero-action preflight. No gameplay is
authorized now. The legacy board remains **30 train · 15 development · 3 fits · 3 comparisons ·
...
## Previous checkpoint: V3 failed action-free; qualify one operational-composition V4 root
...
Code, CI, and preflight are not learning outputs: cumulative
...
1. Run the exact preflight-qualified frozen screen once and record its train-only result.
2. Representation failure records zero fits and triggers a same-evidence collision/stability audit.
3. Mixed or worse leave-one-root-out evidence rejects this design; do not redesign again first.
4. A pass permits only freezing a fresh 12-question Red **design**: trainee/venue ×
...
5. Reorient immediately. No result here grants authority, executes development, opens Crystal, or
...
records the label-free boundary and unchanged counters.
