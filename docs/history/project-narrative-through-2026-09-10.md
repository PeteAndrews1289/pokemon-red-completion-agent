...
# Historical — recovery must happen while options still exist
...
classrooms failed before the choice existed. Building another tiny schedule for exactly the two or
three missing examples would spend another publication and trust-boundary cycle only to risk the
same attrition again. So the next operation did not play Red. It counted what remained.

From exact green main, the emulator reopened 81 authenticated contexts without advancing a frame.
Thirteen consumed roots were excluded. Fifty-nine unused independent roots remained—36 train and
23 development—and 55 could construct at least one genuine three-option menu. Even after the
first batch, the deterministic scheduler could still form a fresh eight-train/four-development
schedule spanning all seven Red-supported portable decision kinds with one lineage per row.

That evidence supports a larger prospective successor: sixteen train lineages and four read-only
development lineages, maximum one scenario per lineage. The old 5/8 yield implies ten settled
examples in expectation, but the system does not use that estimate to pick individual roots and
does not promise the result. The plan must be frozen before any setup rehearsal, random arm, or
outcome. Its namespace and runner identity differ from the consumed pilot, while Red's menu
templates remain the same fifteen cartridge-grounded choices. This is how the project converts a
failure rate into better experimental design without converting failure knowledge into labels.
...
queries were zero. Across all terminals 21 choices were recorded, but the 19 choices in failed
prefixes remain diagnostic-only and are excluded from fitting. The
...
The legacy board remains 30/15/3/3/0/0. Product-focus-v2 is now 12 development attempts, 2 verified
outcomes, 0 atomic episodes, 1 composition attempt, and 1 verified composition; the planned fit has
not happened. This is the first admitted teacher-free same-context Red development result, not
evidence of reliable gameplay, competence, completion, living-Pokédex ability, generalization, or
transfer.

## Previous August 18 checkpoint: replacement V2 reaches a clean starting line
...
measured finite 2,000-trip envelope. Fresh seed 990007 crossed the old failure point, completed team
training, and defeated Agatha. Its task-complete prefix contained 17 targets; the exact frozen model
passed all 17 with 0.07965 cross-entropy while the deterministic resolver reached 12/17. Bruno was
...
| The first v12 rehearsal cleared the moving Cerulean Mart customer and reached the Spearow capture at checkpoint 86, but one requested throw changed the Ball stack from 30 to 28. | The capture-animation loop advanced text with A. If MAIN appeared and disappeared between observations, that same input could select ITEM and issue another throw before the first persistent inventory update was sampled. | Post-throw settling now uses B, which advances battle text but cannot select a MAIN-menu command. Exact one-Ball accounting remains mandatory for every failed or successful throw; the artifact is uncounted and all v12 collection slots remain unopened. |
| The capture-safe v12 replay passed Spearow, Diglett, DUX, Cut, and Dig through checkpoint 91, then Route 1's horizontal youngster blocked the survey's only northbound tile above `(14, 14)`. | The semantic corridor verified ordinary movement retries but did not distinguish a transient source-defined walker from static terrain. | Only at that exact Route 1 map, coordinate, and northward request, the survey may step east, wait, restore the approach, and retry under a 24-attempt bound. Other collisions still fail closed, incidental encounters retain their flee/resource proof, and the artifact remains uncounted. |
| The walker-safe v12 replay passed the entire Surge chapter and reached the Rock Tunnel supply gate at checkpoint 102, but early captures left a ₽2,109 shortfall while only five obsolete Potions remained. | The economy could adapt to several extra throws but still assumed that surplus Balls would finance the next safety reserve. A valid near-worst capture lineage contradicted that hidden luck dependency. | The teacher revisits Route 11, defeats source-pinned Gambler set 1 for exactly ₽1,260 without battle-item recovery, returns to the same Vermilion boundary, and sells unused TM24 for ₽1,000. The fixed Tunnel reserve is preserved. This produces an explicit transferable lesson—earn money through a bounded battle when consumable variance exhausts resale value—and expands the scheduled roster from 68 to 69 battles. |
| V12 qualified 312/312 checkpoints and all 69 scheduled battles, but its first immutable training schedule stopped on the S.S. Anne first floor at `(9, 6)`. | The left/right waiter had occupied `(8, 6)`. Repeating left pinned the two actors face-to-face, so ordinary collision retries could not create a legal passing tile. | The teacher recognizes only that exact ship map, coordinate, and westbound request; steps down to `(9, 7)`, waits under a finite bound, restores `(9, 6)`, and proves the step to `(8, 6)`. The failed v12 root remains sealed. V13 uses wholly new rehearsal and held-out schedules so the repair is tested prospectively. |
| V13's first uncounted rehearsal met Vermilion's harbor sailor at `(22, 27)`, one tile earlier than the prior qualified collision at `(21, 27)`. | The existing yield was tied to one player coordinate even though the same horizontal patrol can block either adjacent westbound step. | The maneuver now accepts exactly those two source-observed gates, derives the matching northward yield and westward clear tiles, and retains the same finite restoration proof. Every other map, coordinate, direction, or failed return remains a hard error. |
| The sailor-safe v13 rehearsal advanced to the Saffron supply lesson at checkpoint 239, where Celadon Mart 5F's vertical gentleman blocked the second westward step from `(15, 2)`. | Ordinary retries left player and NPC sharing the only top aisle; the source object is allowed to walk vertically from its `(14, 5)` origin up to `(14, 2)`. | Only for the X Special approach at that exact map and coordinate, the teacher steps down to `(15, 3)`, waits under 16 bounded attempts, restores `(15, 2)`, and proves entry into `(14, 2)`. Other shop collisions remain errors, and the rehearsal is uncounted. |
| The customer-safe replay reached Cinnabar at checkpoint 272 with 19 effective bag slots and zero Antidotes. | Mansion capacity planning could sell an Antidote when full, but this eight-throw Snorlax lineage had consumed the cure and still retained unused TM21. Rejecting the alternate sale encoded an item-name accident rather than the portable need to free one slot. | The full-bag planner prefers Antidote when present and otherwise sells exactly one unused TM21 for the source-exact ₽2,500. Bide remains available for the delayed TM38 lesson, X Accuracy remains protected, and smaller inventories make no early sale. |
| The alternate Cinnabar sale passed Blaine and reached checkpoint 291, then Indigo preparation could not buy Submission because its customary TM21 capacity sale had already happened. | Capacity was managed locally at Cinnabar without carrying the changed inventory lineage into the later shop. TM14 looked expendable but is reserved for the post-Lance Champion moveset. | Indigo preparation chooses between two declared obsolete stacks: sell TM21 when retained, otherwise sell all remaining 20-HP Potions. It refuses to proceed with neither option, preserves TM14, and frees exactly one bag slot for the same teaching purchases. |
| V13 qualified the full game, but its first counted schedule defeated Agatha and then stopped at checkpoint 305 with a full-HP poisoned Blastoise. | The battle had consumed every Full Heal. Field recovery selected by HP alone, so full HP forced an unavailable Full Heal even though Full Restores remained and legally cure poison. | The failed root is immutable and v13 is retired. V14's post-Agatha selector considers status and live inventory together: use Full Heal when available, otherwise Full Restore; ordinary HP-only damage still requires Full Restore. The receipt now reports the complete recovery state and cause on failure. |
| V14's first uncounted rehearsal cleared Rock Tunnel and healed safely in Lavender, but its evidence gate found zero Awakenings remaining. | The new battle offsets legitimately required three sleep cures, one more than the three-item plan could spend while preserving Pokémon Tower's cure. Lavender Mart does not sell Awakenings, so accepting zero would merely defer the shortage. | Vermilion now purchases two Awakenings on top of the two-item opening reserve, proves a four-item total, accounts for the extra ₽200, and permits up to three observed uses only when one survives. This increases schedule tolerance without another bag slot or a counted attempt. |
| The first four-Awakening replay stopped at Vermilion Mart with three in the bag. | The target and budget used the new four-item reserve, but the purchase call still contained the old literal quantity of one. The live target proof correctly rejected the mismatch before Rock Tunnel. | The purchase quantity now comes from the same declared constant as the cost and reserve calculations, with regression assertions fixing the two-copy purchase and four-copy total. |
| The corrected v14 replay defeated Lance at checkpoint 309 with all six teammates alive, then the Champion handoff rejected an empty Full Restore stack. | Lance legitimately needed a status cure after the two Full Heals were spent and selected the last Full Restore. The old supply mix made a general healing item cover a cheap status-only contingency, violating the explicit Champion reserve. | Indigo now buys six Full Heals in the existing stack, adding four inexpensive status cures while preserving the Full Restore gate for HP recovery. The Champion boundary error now exposes map, position, events, party, moves, HP, and inventory so future failures identify the actual predicate. |
| V14 qualified the full game, but its first counted root met a level-31 Dugtrio during pre-ship training. | RUN preserved PP but was not a safety guarantee: Dugtrio was faster, denied escape, and one-shot the full-health level-26 Wartortle for 71 HP. Fighting had the same first-move hazard, so retry logic could not solve the underlying curriculum choice. | The counted artifact is immutable and v14 is retired. V15 teaches the same level, PP, healing, and repeated-battle skills in Route 11's lower-level grass. Diglett's Cave remains a later capture lesson, separating useful area knowledge from an unnecessary single-member knockout risk. |
| V15 qualified **312/312 checkpoints**, **36/36 objectives**, all **69/69** scheduled trainer battles, Champion, and Hall of Fame with its balanced six-member route, but its first counted root stopped in Route 11 when Venonat disabled Water Gun. | The training selector ranked moves by preferred role and live PP but omitted the already-observed disabled-slot field. It therefore selected a move that existed and had PP but was temporarily illegal. The runtime failed closed before substituting an unrecorded action. | The failed v15 outcome remains sealed. V16 excludes the live disabled slot from the reusable wild-training policy, retains the complete v15 route and team curriculum, and uses entirely new rehearsal and counted schedules. |
...
Buying the third cure required one additional Mart quantity input. The nominal 120-frame pulse
actually advances 144 emulator frames once its bounded press/release overhead is included. Keeping
the old wait shifted the Tunnel lineage and fainted at trainer 5; subtracting only 120 then left a
measured 24-frame surplus and exhausted the Antidote allowance. The alignment wait is therefore 47
...
Its first exact-schedule reproductions proved the branch selection and then showed `(13,3)` was not
...
battle: it withdraws RED's guaranteed PC Potion during the first Pewter Center visit and spends
that single resource after the trainer. The next replay survived the walk, then trainer one's first
opponent left 13/35 HP and the second used repeated Wrap until the lead fainted. A traced battle
repair may spend one additional Potion only at a verified MAIN boundary and protects twelve. The
