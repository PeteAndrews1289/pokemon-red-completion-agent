...
## Current checkpoint: the key fits; now turn it exactly once

The sixteen-card training key is published, rebuilt, and tested against the real lock. GitHub
caught one bookkeeping assumption about squash history, and the production launcher caught a
generated bytecode cache in a disposable runtime. Both stopped before a game decision. After one
bounded bookkeeping repair and a fresh cache-free dependency copy, the cartridge-free preflight
opened training card zero, checked its save and lineage, and closed again without a claim, button,
frame, random choice, outcome, prediction, teacher, or model fit.

So the honest scoreboard is still **CAUSAL TRAIN 6/60 · POWERED FIT 0 · AUTHORITY 0 · TRANSFER
0**. But the blocker has changed. We no longer need another key, census, rehearsal, or teacher
route. The next step is one real classroom: training card zero is claimed once, one random
collection strategy is written down before movement, and only what actually happens to that
strategy can become a lesson. Whether it succeeds or ends as an honest empty desk, we stop and
reorient before opening card one. That discipline is what keeps this on the path to a player that
can eventually complete stories and living Pokedexes across games instead of becoming another
fragile Red walkthrough.
