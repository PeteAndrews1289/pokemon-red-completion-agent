...
1. Capability: choose a useful local registration goal across executable capture and evolution.
2. Learned authority: one sampled model choice controls one played attempt; no teacher replacement.
3. Transfer test: this reset is correlated training only; separate-lineage validation remains required.
4. Cheapest falsifier: source-free provenance tests, then the exact Model121 terminal's action-free
   menu under the same registration policy used by execution.
5. Time box: one session, at most two hours; prefer existing runtime and outcome admission.
6. Stop condition: parent authentication or the two-family menu fails; retain the cause, do not
   select another source, retry a consumed identity, or claim independent evidence.

The implementation must preserve the measured parent's original trust tier and catalog exclusion.
It may prospectively authorize a new bounded training episode, not relabel historical evidence.

## Implemented qualification

- Versioned correlated completion, registered and economy plans bind the measured parent,
  original lineage, terminal state, model, execution profile, reset identity and one-decision cap.
- An exclusive durable claim precedes the gate and survives failures and interruptions. Identical
  sealed-record publication alone is deliberately not treated as a lock.
- The new gate observes the production menu without sampling either authority. Historical
  preflight behavior stays unchanged; the new path requires capture and evolution together.
- The native recorder and loader retain registration/resource outcomes and reject intermediate
...
own native recorder/replay tests. Its review predates that final addition. Flash3.8 High supplied
