...
Each full run expects the same 74 stable public adaptive-battle identities in route order. A
...
is positive. The terminal event must attest a complete 74/74 schedule. These records prove that a
...
registry-declared, disjoint `--schedule-dry-run` must then successfully attest all 74 battles
...
schedule, episode, manifest, and 74/74 offline audit. Every counted slot reopens and re-audits that
