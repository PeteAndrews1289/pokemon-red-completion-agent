...
## 2026-08-30: successor preflight passed — one authentic outcome is the only active gate

The mission is unchanged: build a hierarchical player that completes stories and living Pokedexes
across games, with Red as causal curriculum and Crystal as the first transfer falsifier. The
successor consumer is now published and preflighted; it is still only a lesson collector, not a
trained player.

PR 106 squash-merged the consumer as main `37fbe093`. Its exact-main CI run `33289151289/1`
failed one test because that test assumed the pre-squash feature commit remained reachable. The
executable source passed; the one allowed CI-only repair made the historical provenance assertion
squash-safe without changing source bundle `53116a4a…`. PR 107 merged that repair as exact main
`248346b049fde0ecd70f59277fc31ad9bce9a522`, and push CI `33290594694/1` passed every gate.

The first production launch then rejected a disposable virtual-environment copy containing
generated bytecode at `bootstrap_source_authentication`; it reached neither authenticated source
nor the private root. An inert diagnostic identified the public runtime-cache cause. A fresh
exact-main checkout with cache-free reviewed closure `3dd20373…c62` made the one authenticated
preflight: successor ordinal 0, one selected-root read, no ROM, and **claims/actions/frames/
commitments/outcomes/predictions/teachers/fits 0**. Do not preflight again. The
[path-free result](docs/evidence/red-living-dex-clustered-successor-train-preflight-result-v1-2026-08-29.json)
is `f7e5d8de…aac5`.

The board remains **causal train 6/60 · powered fit 0 · authority 0 · transfer 0**. Publish this
receipt and require green exact-main CI, then run successor train ordinal 0 exactly once. Its
logical and physical pair must be claimed before input, one randomized arm must remain durable,
and only that arm's independently observed outcome may become a target. Setup failure or
interruption is an honest consumed no-target terminal, never a retry. Stop and reorient before
ordinal 1 or any fit. Development, sealed Red, Crystal, teacher targets, counterfactuals, and full
