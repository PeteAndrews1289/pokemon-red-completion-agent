...
## First real navigation outcome probe frozen, not executed — 2026-08-14
...
The next experiment is prospectively frozen in
...
It uses the authenticated, non-sealed scenario-001 shared-clear Cerulean state and one exact
...
coordinate. Both plans are computed before any controller input. Candidate order is derived from
...
Execution must wait for publication and green exact-commit CI. It then opens a deterministic
private artifact before candidate zero, reloads identical state bytes for each candidate, executes
each exactly once, appends each typed success/failure immediately, and adapts the pair through
`navigation.route.arrival-efficiency.v1`. A typed route failure is evidence and is not rerun. This
is an adapter/plumbing result only: no model fit, authority, teacher target, sealed Red case,
Crystal context, or full replay is authorized. After its one result, build the bounded real
party-development outcome; do not turn this short route into a route-specific hardening campaign.
