#!/usr/bin/env python3
"""Strictly preflight the exact five-root Red development batch without gameplay.

This successor command preserves the historical train consumer byte-for-byte.
It authenticates published source and exact-main CI before project imports,
stages the reviewed dependency closure, then admits exactly two historical
and three supplement development roots. It exposes no ROM, controller,
prediction, claim, teacher, outcome, fitting, train-row, or retry interface.
"""
...
FAILURE_SCHEMA = "pokemon.red.living-dex-development-batch-preflight-failure.v2"
...
class DevelopmentBatchCommandError(RuntimeError):
    """The successor command failed at one sanitized, action-free stage."""
...
def _require_arguments(args: argparse.Namespace) -> tuple[str, str, int, int]:
    identity = (
    ...
    if (
        _BOOTSTRAP_IDENTITY is None
        or identity != _BOOTSTRAP_IDENTITY
        or not isinstance(identity[0], str)
        or _GIT_COMMIT.fullmatch(identity[0]) is None
        or not isinstance(identity[1], str)
        or _SHA256.fullmatch(identity[1]) is None
        or type(identity[2]) is not int  # noqa: E721
        or identity[2] <= 0
        or type(identity[3]) is not int  # noqa: E721
        or identity[3] <= 0
    ):
        raise DevelopmentBatchCommandError("bootstrap_source_authentication")
    return identity
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        identity = _require_arguments(args)
        ...
        stage = "post_preflight_authentication"
...
def _require_runtime_postcheck() -> None:
    ...
    if (
        ...
        raise DevelopmentBatchCommandError("runtime_postauthentication")
    _require_environment(
...
def _filesystem_git_worktree_probe(path: Path) -> bool:
    try:
        resolved = path.resolve(strict=True)
        return any(
            os.path.lexists(ancestor / ".git")
            for ancestor in (resolved, *resolved.parents)
        )
    except OSError:
        raise DevelopmentBatchCommandError("private_root_authentication") from None
