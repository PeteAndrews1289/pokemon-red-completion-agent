...
def _generated_payloads() -> tuple[bytes, bytes, dict[str, object]]:
    ...
    document["collection_id"] = "red-battle-heldout-v17"
    document["runs"] = [
        {
            "harness_seed": seed,
            "partition": partition,
            "run_id": f"red-battle-v17-{ordinal:02d}-{partition}",
        }
        for ordinal, (partition, seed) in enumerate(
            (
                *(("train", seed) for seed in range(27_001, 27_006)),
                *(("validation", seed) for seed in range(37_001, 37_003)),
                *(("test", seed) for seed in range(47_001, 47_006)),
            ),
            start=1,
        )
    ]
    ...
    document["schedule_dry_run"] = {
        "dry_run_id": "red-battle-schedule-dry-run-v17",
