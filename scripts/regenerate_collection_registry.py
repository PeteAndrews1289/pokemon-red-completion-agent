...
def _generated_payloads() -> tuple[bytes, bytes, dict[str, object]]:
    ...
    document["collection_id"] = "red-battle-heldout-v14"
    document["runs"] = [
        {
            "harness_seed": seed,
            "partition": partition,
            "run_id": f"red-battle-v14-{ordinal:02d}-{partition}",
        }
        for ordinal, (partition, seed) in enumerate(
            (
                *(("train", seed) for seed in range(24_001, 24_006)),
                *(("validation", seed) for seed in range(34_001, 34_003)),
                *(("test", seed) for seed in range(44_001, 44_006)),
            ),
            start=1,
        )
    ]
    ...
    document["schedule_dry_run"] = {
        "dry_run_id": "red-battle-schedule-dry-run-v14",
