...
def _generated_payloads() -> tuple[bytes, bytes, dict[str, object]]:
    ...
    document["collection_id"] = "red-battle-heldout-v16"
    document["runs"] = [
        {
            "harness_seed": seed,
            "partition": partition,
            "run_id": f"red-battle-v16-{ordinal:02d}-{partition}",
        }
        for ordinal, (partition, seed) in enumerate(
            (
                *(("train", seed) for seed in range(26_001, 26_006)),
                *(("validation", seed) for seed in range(36_001, 36_003)),
                *(("test", seed) for seed in range(46_001, 46_006)),
            ),
            start=1,
        )
    ]
    ...
    document["schedule_dry_run"] = {
        "dry_run_id": "red-battle-schedule-dry-run-v16",
