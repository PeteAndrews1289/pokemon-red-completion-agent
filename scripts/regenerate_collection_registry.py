...
def _generated_payloads() -> tuple[bytes, bytes, dict[str, object]]:
    ...
    document["collection_id"] = "red-battle-heldout-v60"
    document["runs"] = [
        {
            "harness_seed": seed,
            "partition": partition,
            "run_id": f"red-battle-v60-{ordinal:02d}-{partition}",
        }
        for ordinal, (partition, seed) in enumerate(
            (
                *(("train", seed) for seed in range(820_001, 820_006)),
                *(("validation", seed) for seed in range(830_001, 830_003)),
                *(("test", seed) for seed in range(840_001, 840_006)),
            ),
            start=1,
        )
    ]
    ...
    document["schedule_dry_run"] = {
        "dry_run_id": "red-battle-schedule-dry-run-v60",
