...
def _capture_paths(catalog_root: Path, checkpoint_id: str) -> tuple[Path, Path]:
    ...
    if not state.is_file() or not envelope.is_file():
        raise RedPartyPpPreflightRunError(
            "reserved PP source is missing its state or envelope"
        )
...
def _required_party_vectors(raw: RawGameState) -> tuple[
    tuple[int, ...],
    tuple[int, ...],
    tuple[int, ...],
    tuple[int, ...],
    ...
    if (
        raw.party_count != 6
        or any(value is None or len(value) != raw.party_count for value in values)
    ):
        raise RedPartyPpPreflightRunError(
...
def _venue_binding_sha256(
    ...
    *,
    ...
    if evidence is None:
        raise RedPartyPpPreflightRunError(
            "two-prior registry lacks the shared preparation venue"
        )
    return canonical_sha256(
        {
            "schema": "pokemon.red.party-development-pp-venue-binding.v1",
...
def _run(args: argparse.Namespace) -> dict[str, object]:
    paths = {
        ...
        "out_plan": _require_external(
            args.out_plan, subject="PP materialization plan"
        ),
        "out_summary": _require_external(
            args.out_summary, subject="PP materialization summary"
        ),
    }
    if len(set(paths.values())) != len(paths):
        raise RedPartyPpPreflightRunError(
            "PP materialization preflight paths must be distinct"
        )
    ...
    for reservation in reservations:
        try:
            inventory_entry = inventory_by_checkpoint[
        ...
        if (
            ...
            or reservation.source_state_sha256
        ...
        complete_party_experience = tuple(
            value for value in party_experience if value is not None
        )
        adapter = _start_adapter(raw)
        species, levels, hp, maximum_hp, status, moves, pp = (
            _required_party_vectors(raw)
        )
        ...
        safe_current_pp = sum(
            pp_state.moves[slot - 1].current_pp for slot in safe_slots
        )
        target_national_number = red_internal_species_number(
            species[target_index]
        )
        ...
        sources.append(
            RedPpMaterializationSource(
                ...
                target_has_evolution_route=bool(
                    cartridge_evolutions.get(target_national_number)
                ),
    ...
    if rom_adjacent_artifacts(rom_path) != adjacent_before:
        raise RedPartyPpPreflightRunError(
            "read-only PP preflight created a ROM-adjacent artifact"
        )
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        parser.error(
            "Red PP materialization preflight failed closed; private paths were withheld"
        )
