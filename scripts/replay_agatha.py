"""Run Agatha from a private captured pre-chapter state.
...
Usage::
    ...
    POKEMON_RED_ROM=<path> python scripts/replay_agatha.py --state <path>.state \\
...
from pokemon_red_completion.agatha import run_agatha_chapter  # noqa: E402
...
def main(argv: list[str] | None = None) -> int:
    ...
    parser.add_argument("--state", type=Path, required=True, help="private pre-Agatha state")
    parser.add_argument(
        "--out-state",
        type=Path,
        default=None,
        help="optional private post-Agatha state for the next chapter",
    )
    ...
    try:
        ...
        try:
            report = run_agatha_chapter(emulator, reader, executor)
