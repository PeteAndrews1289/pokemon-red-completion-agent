...
| Cheapest falsifier | A ROM-free purpose-built capture contract must prove on adversarial fixtures that prospective Red captures can target the measured menu, option-kind, family, and location deficits while setup actions remain outside learner targets, roots stay repeatable and unique, and the unchanged 8+4 coverage gate remains exact. |
...
**2026-08-26-red-exhaustive-inventory-v3-exact-coverage-shortfall** · status **active** · evidence [qualification](docs/evidence/red-living-dex-exhaustive-inventory-v3-failure-2026-08-26.json)

| Check | Session conclusion |
| --- | --- |
| Product alignment | The exhaustive result ends repeated archaeology on a bank designed for a different purpose. Prospective captures aim directly at the transferable product: complete semantic living-Pokedex choices whose selected consequences can train a shared option-value model, while Red-specific setup and observation remain adapter concerns rather than model targets. |
| Learning output | No new model example. V3 reconciled all 81 contexts, projected eight complete menus, and evaluated exact coverage once. The legacy bank measured train 5, development 3, option kinds 3, and one location per partition; 65 contexts had fewer than three options, four failed enumeration, and four roots were consumed. Protected effects were zero. Counters remain development 15, verified 5, causal train 0, fits 4, comparisons 4, authority 0, and transfer 0. |
| Authority delta | None. The V3 census made no behavior draw, controller-authority attempt, game action, frame advance, outcome observation, model prediction, teacher query, fit, authority promotion, or transfer result. It published no private plan or identity/path field. |
...
| Blocker | The complete legacy bank contains only five train and three development complete menus, only three offered option kinds, and one location per partition. It cannot meet the unchanged 8+4 multi-kind, multi-family, location-disjoint curriculum. No qualified prospective capture factory yet creates the missing genuine living-Pokedex decisions. |
| Decision | Retire V1, V2, and V3 without retry and stop mining the legacy bank. Build a ROM-free purpose-built capture contract and coverage specification that prospectively targets the measured deficits, keeps deterministic setup actions outside learner labels, preserves repeatable unique roots, and admits only independently observed selected-arm outcomes. Publish and reorient before any new gameplay capture. |
| Next session | Implement and qualify only the purpose-built capture schema, deficit-aware coverage planner, setup-versus-decision provenance boundary, repeatable-root ledger, path-free receipt, and adversarial tests. Publish through exact-source CI and reorient before opening a ROM/private capture or running any setup policy, selected option, outcome, fit, Crystal episode, sealed Red case, or full replay. |
| Next falsifier | Adversarial ROM-free fixtures must prove that the prospective capture contract rejects incomplete or identity-bearing menus, setup-policy labels, reused or one-shot roots, train/development family or location overlap, outcome-dependent capture selection, counter mismatch, interruption ambiguity, and any weakening of eight train across four kinds and three families plus four disjoint development scenarios. |
| Stop condition | Stop the capture qualification before protected access if it cannot distinguish deterministic setup provenance from learner decisions, guarantee complete 3+ option menus and repeatable unique roots, encode the exact deficit-aware 8+4 split without overlap, preserve anonymous public receipts, or fail closed on interruption and counter mismatch. V1/V2/V3 never retry; outcome collection, sealed Red, Crystal, promotion, and replay remain closed. |

### Stop conditions

- Stop before new gameplay capture until a published purpose-built capture contract proves complete 3+ option menus, genuine executors, repeatable unique nonsealed roots, deterministic-setup isolation, eight train scenarios across four option kinds and three families, and four development scenarios disjoint from train by family and location. V1, V2, and V3 may not retry.
