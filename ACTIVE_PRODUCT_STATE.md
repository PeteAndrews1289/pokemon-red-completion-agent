...
**2026-08-27-one-slot-wrapper-qualified-publication-gate** · status **active** · evidence [qualification](docs/evidence/red-living-dex-claim-first-one-slot-invocation-v1-local-qualification-2026-08-27.json)

| Check | Session conclusion |
| --- | --- |
| Product alignment | The one-slot wrapper is the shortest remaining bridge from the frozen Red classroom to a genuine selected-arm lesson: it binds current clean source and exact CI, authenticates the sealed producer/private joins, reads one root without sibling inventory coupling, owns the only Red resolver selection, and cannot claim or construct runtime in preflight. The reusable ledger remains title-neutral; this Red adapter is not Crystal model transfer. |
| Learning output | No new model example. Sixty-seven focused and 5,389 repository tests qualify infrastructure only. Counters remain development 15, verified 5, causal train 0, fits 4, comparisons 4, authority 0, and transfer 0. |
| Authority delta | None. No ROM or authentic private capture was opened, no root was claimed, no controller or frame moved, and no provider, teacher, prediction, outcome, fit, promotion, or transfer ran. |
| Transfer result | Not run. The title-neutral account claim primitive still passes Red-shaped and Crystal-shaped fixtures, while this invocation owns a deliberately Red-specific cartridge adapter. That is safe architectural separation, not Crystal model transfer. |
| Blocker | The current-source one-slot wrapper is locally qualified but not yet published under exact successful push CI, and its one real selected-root preflight-only rehearsal has not run. Private setup remains closed until publication, that sole structural rehearsal, and an immediate reorientation complete. |
| Decision | Publish the audited wrapper and source-bound registries as one exact milestone, require successful exact-head push CI, perform exactly one ROM-free preflight-only rehearsal of one never-claimed selected root, publish only its path-free structural receipt, and reorient immediately. Do not create another infrastructure-only gate: the following decision must open the shortest setup-to-randomized-outcome sequence or abort this lane. |
| Next session | Publish the exact wrapper, require successful push CI, run one ROM-free selected-root preflight-only rehearsal, and reorient in the same session. Stop before setup execution, but do not authorize another infrastructure-only session afterward. |
| Next falsifier | The exact published command must authenticate current source and exact push CI before project import, reopen the immutable producer record, read only one selected state/envelope pair, observe both roots available, disclose no private identity or path, and leave claims, runtime construction, controller effects, learner effects, and model effects structurally unreachable. |
| Stop condition | Stop on failed exact-source publication or CI, wrapper access to an unselected sibling, a claim or runtime during preflight, an unauthenticated producer/private join, identity leakage, or any private setup execution before reorientation. After one successful rehearsal, stop this infrastructure gate permanently and reject another wrapper-only session. |
