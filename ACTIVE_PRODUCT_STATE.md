...
| Cheapest falsifier | Before another authenticated read, a ROM-free V2 diagnostic qualification must distinguish every private stage and exact 8+4 coverage failure with path-free aggregate counters while keeping effects at zero. Then one new-identity action-free census must either prove the unchanged curriculum gate or stop with an exact reason. |
...
**2026-08-26-red-authentic-inventory-failed-closed** · status **active** · evidence [qualification](docs/evidence/red-living-dex-authentic-inventory-failure-v1-2026-08-26.json)

| Check | Session conclusion |
| --- | --- |
| Product alignment | The V1 census reached the shortest authentic bridge and failed safely, but its broad receipt discarded the bounded cause. Preserving exact failure information is the smallest maintenance unblock for the first causal living-Pokedex model; rerunning blindly or lowering coverage would repeat the project's documented focus drift. |
| Learning output | No new measured learner output. Source 98d4f21a passed exact-main CI with 4,831 tests, and V1 authenticated all 81 private inputs before failing closed. The path-free receipt records draws 0, actions 0, frames 0, claims 0, outcomes 0, predictions 0, teachers 0, and fits 0. Counters remain development 15, verified outcomes 5, causal train 0, model fits 4, unseen comparisons 4, authority 0, and transfer 0. |
| Authority delta | None. The action-free attempt made no model prediction, behavior draw, game action, outcome observation, fit, authority promotion, or transfer result. |
...
| Blocker | V1 failed closed at the broad action_free_inventory stage with every protected effect at zero, no plan, and no materializer namespace. Its receipt did not retain whether authentication, namespace state, historical replay, menu projection, or exact 8+4 coverage caused the stop, so neither a coverage success nor a coverage shortfall is established. |
| Decision | Retire the V1 inventory identity without retry. Qualify a ROM-free V2 diagnostic boundary that assigns finite path-free reason codes and aggregate coverage/effect counters before a new published V2 identity may read authenticated captures. Keep the exact 8+4 kind, family, location, provenance, and disjointness gates unchanged. |
| Next session | Implement and qualify only the V2 path-free diagnostic boundary. Publish it through exact-source CI, then reorient before deciding whether one new-identity action-free authenticated census is justified. Do not reopen V1, collect an outcome, fit, run Crystal, open sealed Red, or replay the game. |
| Next falsifier | ROM-free tests must kill collapsed authentication, namespace, replay, projection, partition-count, offered-kind, family-count, location-count, overlap, state-read, exclusion-count, and effect-counter mutations. Any identity/path leak or ambiguous survivor stops V2 before protected access. |
| Stop condition | Stop V2 before protected access if any failure class remains ambiguous, any public field can carry a private identity or path, or any zero-effect counter can be falsely reported. A later authenticated V2 census stops on the first exact failure and never weakens the 8+4 gate. Outcome collection, fitting, sealed Red, Crystal, promotion, and full replay remain closed. |

### Stop conditions

- Stop before real gameplay unless one new-identity action-free authenticated inventory proves complete menus, genuine executors, unique repeatable roots, eight train scenarios across four option kinds and three families, and four development scenarios disjoint from train by family and location. V1 may not retry, and V2 must first pass ROM-free diagnostic qualification.
