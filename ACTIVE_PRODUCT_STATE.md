...
| Cheapest falsifier | One new-identity V3 action-free authenticated census must reconcile the complete 81-context bank and either freeze the unchanged 8-train plus 4-development curriculum or stop once with exact aggregate exclusions and coverage while every protected effect stays at zero. |
...
**2026-08-26-red-exhaustive-inventory-v3-published** · status **active** · evidence [qualification](docs/evidence/red-living-dex-exhaustive-inventory-v3-qualification-2026-08-26.json)

| Check | Session conclusion |
| --- | --- |
| Product alignment | V3 turns the V2 blocker into the shortest decisive classroom test: it can inspect the whole authentic bank without letting one bad Red-specific adapter context hide later title-neutral living-Pokedex choices. The terminal will either open the first causal collection campaign or prove purpose-built captures are required without lowering the learner's curriculum. |
| Learning output | No new measured learner output. Main 1db95546 passed exact-main CI with 4,981 tests. Seventy-one focused and 213 related tests prove continued iteration, fatal-stop non-downgrade, hidden-effect retention, exact whole-bank accounting, privacy, and the unchanged gate. Qualification opened no ROM/private capture. Counters remain development 15, verified 5, causal train 0, fits 4, comparisons 4, authority 0, and transfer 0. |
| Authority delta | None. V3 qualification used no private input, model prediction, behavior draw, controller-authority attempt, game action, frame advance, outcome observation, fit, authority promotion, or transfer result. |
...
| Blocker | The exhaustive V3 boundary is published, but the authentic bank has not been reopened under it. Whether the full 81-context bank contains the unchanged 8+4 complete-menu curriculum or instead requires purpose-built living-Pokedex captures remains unknown. |
| Decision | Permit exactly one new-identity V3 action-free authenticated census from published main 1db95546 under exact-main green CI 32946811445. It must reconcile every authenticated context and may freeze a private plan only if the unchanged gate passes; otherwise its exact aggregate exclusions and coverage are the result. V1, V2, and V3 never retry. |
| Next session | Invoke published V3 exactly once against the authenticated repeatable, nonsealed, unconsumed Red bank. Publish only its path-free terminal and reorient immediately. Do not retry V1/V2/V3, collect an outcome, fit, run Crystal, open sealed Red, or replay the game. |
| Next falsifier | The sole V3 census must account for all 81 authenticated contexts as fixed exclusions or complete menus, perform exactly one zero-effect check per restore attempt, evaluate coverage once, and either publish the exact 8+4 plan or return exact aggregate exclusions and the finite coverage status. Any ambiguity, counter mismatch, leak, or protected effect stops. |
| Stop condition | Stop after the first V3 terminal, whether it is a frozen plan, exact coverage shortfall, or global failure. Never retry any inventory identity or weaken the 8+4 gate. Stop immediately on incomplete accounting, ambiguity, identity/path leakage, counter mismatch, or any controller authority, action, frame, draw, claim, outcome, prediction, teacher query, or fit. Outcome collection, sealed Red, Crystal, promotion, and full replay remain closed. |

### Stop conditions

- Stop before real gameplay unless the sole published V3 action-free authenticated census accounts for the complete bank and proves complete menus, genuine executors, unique repeatable roots, eight train scenarios across four option kinds and three families, and four development scenarios disjoint from train by family and location. V1, V2, and V3 may not retry.
