...
- Kind: **learning**
- Rigor: **development**
- Next decision: The recovery/readiness/fitting connection is merged and passed exact-main CI. Fix the launch reader's two-reserve assumption to match the frozen one-reserve plan, validate real pre-input replay, and collect eight bounded train lessons with automatic admitted fitting. Do not refreeze the plan, repeat terminal trials, or count publication work as learning.
...
**2026-09-06-red-training-launch-repair** · status **active** · evidence [qualification](docs/evidence/red-training-retired-bank-freeze-2026-09-06.json)
...
| Blocker | PRs 228 and 229 are merged; exact-main CI 34015856963 passed on ba5a01d0. The first real launch stopped before input because the reader incorrectly required exactly two reserve roots while the unchanged producer plan validly has one. Repair the reader to accept the producer's nonempty reserve contract, retaining eight train slots, four paired roots and all separation checks. No trial, new outcome or production fit was consumed. |
| Decision | The four-hour engineering reassessment is reached. Limit the remaining repair to the proven producer-reader mismatch and validate the full real action-free replay before another publication. Then collect the same eight lessons and let the shared factual gate admit one existing-model update. Preserve all 23 baseline rows and every old terminal; do not open another architecture, CI redesign or teacher-factory project. |
| Next session | Qualify the batched repair, reuse the completed 4/4/1 freeze and collect eight bounded train lessons with automatic admitted fitting. Then report the actual model update and prepare short model-chosen Red goal episodes using deterministic low-level skills. |
| Next falsifier | The qualified real command must action-free rebind the existing 4/4/1 plan and authenticate all 23 retained train rows plus the current eighteen-example prior. Actual eight-slot collection must provide six or more settled rows, acquisition at least one and development at least three; the existing fitter must preserve negatives, baseline rows and immutable recovery. |
