...
| Cheapest falsifier | A ROM-free Red setup-composition plan must fail unless all fifteen frozen slots terminally account, all fourteen routed slots bind exact private route/terminal/provider attestations without exposing identity, every planned semantic option remains genuinely bindable, and one durable runner claims before controller input and forbids retry after input. A synthetic route, provider, menu arm, or successful-looking incomplete ledger is a failure. |
...
**2026-08-26-routed-seam-published-concrete-setup-next** · status **active** · evidence [qualification](docs/evidence/red-routed-semantic-plan-requalification-2026-08-26.json)

| Check | Session conclusion |
| --- | --- |
| Product alignment | The project now has both a statistically adequate Red lesson schedule and the reusable route-to-semantic-skill abstraction later titles need. The next Red-specific setup layer turns that architecture into repeatable causal lesson states without teaching button sequences; it is the shortest path to eight authentic train outcomes while preserving the cross-game living-Pokedex target. |
| Learning output | No new model example. The published seam passed 36 focused and 168 related tests; the requalified unchanged 10+5 Red schedule still reaches 2144/2187, or 98.0338 percent, selected-kind coverage after any two train censors. Pilot execution remains false. Counters remain development 15, verified 5, causal train 0, fits 4, comparisons 4, authority 0, and transfer 0. |
| Authority delta | None. Composition and plan requalification used no ROM/private capture, behavior draw, learner claim, controller authority/action, frame advance, learner label, outcome, prediction, teacher query, fit, authority promotion, or transfer result. They authorize neither Red setup execution nor model training. |
| Transfer result | Not run. The published core composition contract is title-neutral and the Red adapter is thin, but no private Red setup or Crystal adapter has executed. Crystal stays closed until authentic Red causal evidence and a powered Red benchmark justify a frozen-weight transfer test. |
| Blocker | The generic routed-semantic seam is published and the exact Red fifteen-slot schedule now credits it, but pilot execution remains closed. Fourteen slots still need concrete private route/terminal/provider bindings and all fifteen need one durable setup runner. Trade remains a separate later full-living-Pokedex gap; outcomes, fitting, sealed Red, and Crystal remain closed. |
| Decision | Accept the published composition seam and the requalified plan while preserving its execution-blocked verdict. Build only the ROM-free concrete Red setup-composition contract and durable whole-slot runner next. Require exact slot, route-terminal, destination-provider, claim, budget, terminal-accounting, and privacy joins; publish and reorient before any private setup execution. |
| Next session | Implement and adversarially qualify only the ROM-free concrete Red setup-composition schema and durable fifteen-slot runner. Bind all fifteen frozen plan slots, require fourteen routed terminal/provider attestations, preserve complete menus and the 10+5 reserve, and make every whole-slot terminal durable. Publish and reorient before opening private routes, controller input, captures, outcomes, fitting, sealed Red, Crystal, or replay. |
| Next falsifier | The concrete setup contract must reject a missing or duplicate slot, a route or destination attestation joined to another slot, a planned option without a genuine provider, an identity-bearing public receipt, controller input before durable claim, any claimed-slot retry, an unaccounted failure or interruption, and any setup label, behavior draw, learner outcome, prediction, teacher query, or fit. Passing ROM-free fixtures must not imply that a private Red route has been authenticated. |
| Stop condition | Stop if the concrete setup layer needs a teacher route, fixed walkthrough label, synthetic menu arm, identity-bearing policy feature, unclaimed controller input, claimed-slot retry, or incomplete terminal ledger. Do not open private setup to rescue a failing ROM-free contract. Outcomes, fitting, sealed Red, Crystal, promotion, and replay remain separate closed gates. |

### Stop conditions

- Stop before new gameplay capture until every frozen Red slot has a qualified concrete setup binding and one durable whole-slot runner. The published seam already preserves destination semantics; the next layer must claim before input, account every terminal, forbid retry after input, and expose no private route, teacher sequence, setup label, or synthetic arm. V1, V2, and V3 may not retry.
