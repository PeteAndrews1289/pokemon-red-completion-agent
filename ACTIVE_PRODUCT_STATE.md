...
**2026-08-27-setup-bridge-namespace-successor-qualified** · status **active** · evidence [qualification](docs/evidence/red-living-dex-setup-bridge-namespace-successor-v2-qualification-2026-08-27.json)

| Check | Session conclusion |
| --- | --- |
| Product alignment | The namespace repair is maintenance only because it is the shortest remaining prerequisite for trustworthy same-root Red decision captures. It adds no Red route behavior and preserves the title-neutral menu, family, outcome and learner surfaces needed for later frozen-weight Crystal transfer. |
| Learning output | No new model example. The maintenance successor passed 71 focused, 234 related, and 5300 full-suite tests plus independent read-only review; no private input or protected effect occurred. Counters remain development 15, verified 5, causal train 0, fits 4, comparisons 4, authority 0, and transfer 0. |
| Authority delta | None. The published runtime-free bridge and its failed rehearsal made no gameplay choice, moved no controller or frame, executed no provider, claimed no root, queried no teacher, predicted or observed no outcome, fit no model, and promoted no authority. |
| Transfer result | Not run. The repair is deliberately store-layout-generic and changes no learner-facing schema; model authority and transfer remain zero. Crystal stays closed until authentic Red causal evidence and a powered Red benchmark exist. |
| Blocker | The namespace successor is locally qualified and both read-only reviewers approve publication plus one rehearsal. Exact publication and green push CI still gate the sole pre-controller proof; controller-capable outer identity, atomic claims, recovery, runtime construction, setup captures, and causal outcomes remain closed until it succeeds. |
| Decision | Publish the minimal namespace successor exactly as qualified, require exact green push CI, and rehearse once against the intended nearest initialized store. V1 never retries; a failed successor ends this namespace lane rather than opening V3. Reject scanning, plan regeneration, storage flattening, and controller-capable work before success. |
| Next session | Freeze the exact qualified commit, publish it, require exact green push CI, perform exactly one read-only pre-controller rehearsal, and reorient immediately. Do not begin the outer controller-capable layer unless that rehearsal succeeds. |
| Next falsifier | The exact published successor must reproduce the 15/45/33/10 frozen join once from the intended shared nearest store while every protected effect stays zero. Any namespace ambiguity, absent or invalid plan, identity drift, leak, effect, or incomplete capacity terminates the lane without V3. |
| Stop condition | Stop after the single successor rehearsal, or immediately on namespace ambiguity, absent/invalid plan, identity drift, protected effect, leak, or incomplete 15/45/33/10 capacity. A failed successor ends this namespace lane: do not create V3; reorient to causal learning. Do not scan siblings, regenerate the plan, flatten storage, claim a root, construct a runtime, execute setup, collect an outcome, fit, open sealed Red or Crystal, promote, or replay. |
