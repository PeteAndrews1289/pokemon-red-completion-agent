...
| Cheapest falsifier | The concrete private Red source adapter and published action-free materializer must fail unless approved private inputs produce exactly the frozen fifteen-slot order, all forty-five genuine semantic option-provider joins, one local and fourteen routed slots, exact route-terminal-provider provenance, and a path-free public census with zero protected-effect deltas. A missing, duplicate, cross-joined, changed, synthetic, actionful, or identity-leaking binding is a failure. |
...
**2026-08-26-materializer-published-private-adapter-next** · status **active** · evidence [qualification](docs/evidence/red-living-dex-setup-binding-materialization-v1-qualification-2026-08-26.json)

| Check | Session conclusion |
| --- | --- |
| Product alignment | The project now has the statistically adequate Red lesson schedule, reusable route-to-semantic composition, the durable no-retry setup runner, and an action-free whole-plan materializer. A concrete Red source adapter and one authentic zero-action plan freeze are the remaining bridge to decision-rich setup captures and the first eight causal train outcomes, while the learner-facing choices remain title-neutral for later Crystal transfer. |
| Learning output | No new model example. The materializer passed 12 direct, 94 related setup/plan/protocol, 49 protocol, and 5,087 full-suite tests. It derives and seals all fifteen slots and forty-five option joins only from synthetic ROM-free source fixtures; no authentic source adapter or private plan was used. Counters remain development 15, verified 5, causal train 0, fits 4, comparisons 4, authority 0, and transfer 0. |
| Authority delta | None. Materializer qualification used only synthetic ROM-free source fixtures and no private capture, ROM, controller action, frame advance, behavior draw, learner claim, label, outcome, prediction, teacher query, fit, authority promotion, or transfer result. It authorizes neither actionful Red setup nor model training. |
| Transfer result | Not run. The materializer boundary is reusable and keeps title-private bindings behind a shared action-free source shape, but the concrete Red adapter, actual Red plan, and any Crystal adapter remain unexecuted. Crystal stays closed until authentic Red causal evidence and a powered Red benchmark justify a frozen-weight transfer test. |
| Blocker | The reusable route-to-semantic seam, durable fifteen-slot no-retry runner, and action-free whole-plan materializer are published. The concrete private Red source adapter is still missing and no authentic fifteen-slot plan has been read, authenticated, or sealed. Trade remains a separate later full-living-Pokedex gap; actionful setup, outcomes, fitting, sealed Red, and Crystal remain closed. |
| Decision | Accept the published action-free materializer and close that ROM-free engineering gate. Implement and adversarially qualify only the concrete private Red source adapter without protected access, then use a separate action-free gate to freeze one exact fifteen-slot plan from approved private inputs. Publish only aggregate evidence and reorient before invoking the setup runner. |
| Next session | Implement and adversarially qualify only the concrete private Red setup-binding source adapter without private access. Then, under a separately recorded action-free gate, authenticate approved private inputs and freeze one complete fifteen-slot plan with all forty-five genuine arms and fourteen routed joins. Publish aggregate evidence and stop before setup execution, behavior draw, outcome collection, fitting, sealed Red, Crystal, promotion, or replay. |
| Next falsifier | The concrete adapter plus materializer must reject any source/input-set change, slot-order change, missing or duplicate binding, route-terminal-provider cross-join, unavailable semantic provider, teacher-direction route, protected-effect delta, plan replacement, and public path or identity leak. A structurally passing synthetic fixture cannot count as an authenticated private Red binding. |
| Stop condition | Stop if the concrete source adapter or private plan freeze needs controller input, frame advance, a teacher route, fixed directions, a synthetic menu arm, an identity-bearing public feature, a changed protected input, a changed frozen slot, or a retry. Do not invoke the setup runner in this gate. Outcomes, fitting, sealed Red, Crystal, promotion, and replay remain separate closed gates. |

### Stop conditions

- Stop before new gameplay capture until the concrete private Red source adapter is qualified and one action-free materialization authenticates every frozen slot against the published durable whole-slot runner. It must bind all forty-five genuine semantic arms and fourteen routed terminal/provider joins with zero protected-effect deltas and no public identity. The plan-freeze identity may not retry after any protected effect.
