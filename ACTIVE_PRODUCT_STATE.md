...
| Blocker | Main 06d602bb and CI 33821766312 qualified the index repair. Trial 1 completed develop_team with verified semantic change in 2009 actions and 162918 frames while preserving all 15 specimens, but independent admission rejected a reader-only assumption that specialist controller rows link to a standalone goal decision. The immutable episode is not training data until the null-link admission repair publishes and passes CI. |
| Decision | Publish and green the narrow admission repair, require every specialist controller row in this standalone-goal episode to have a null decision_id, and reject forged non-null links. Reopen and admit the immutable trial-1 episode without gameplay, then continue untouched trials 2-8 and fit completed admitted outcomes only. |
...
| Next falsifier | Require green exact-head CI for the null-link admission repair, then reopen the existing trial-1 episode and prove its one decision, 2009 unlinked execution rows, 162918-frame sum, terminal, collection ledger, and one-hot arm all admit without controller input. |
