...
| Cheapest falsifier | One action-free authenticated capture inventory must either prove a path-free 8-train plus 4-development plan across the frozen kind, family, and disjointness gates or fail closed with every claim, draw, action, frame, outcome, and fit counter at zero. |
...
**2026-08-26-red-observed-arm-materializer-published** · status **active** · evidence [qualification](docs/evidence/red-living-dex-durable-materializer-qualification-v1-2026-08-26.json)

| Check | Session conclusion |
| --- | --- |
| Product alignment | The Red-specific materializer now makes each causal living-Pokedex lesson restart-safe without changing the portable feature or outcome contract. The shortest remaining bridge to real learning is proving that existing authentic captures provide enough diverse semantic decisions, not adding more durability layers or replaying the game. |
| Learning output | No new measured learner output. Twenty-four materializer tests and a 55-test related slice prove claim-before-randomization, persisted selection, one authenticated arm, bound observation, exact restart, tamper rejection, and permanent terminals. Exact-source CI passed 4,802 tests. Counters remain development 15, verified outcomes 5, causal train 0, model fits 4, unseen comparisons 4, authority 0, and transfer 0. |
| Authority delta | None. The published Red materializer is ROM-free engineering. It produced no gameplay prediction, selected game action, observed cartridge outcome, model fit, authority promotion, or transfer result. |
| Transfer result | Not run. The learner feature and outcome schema is portable, while the materializer and raw observation provenance are intentionally Red-specific. Crystal needs its own adapter into the shared contract; it remains closed until calibrated Red evidence and a powered Red benchmark justify frozen-weight transfer. |
| Blocker | The restart-safe Red materializer is published, but no action-free authenticated inventory yet proves that repeatable, nonsealed, unconsumed private captures can supply one complete 8-train plus 4-family-and-location-disjoint-development plan across four genuine train option kinds and three train families. |
| Decision | Open exactly one action-free authentic capture inventory and plan preflight. Derive and publish the path-free 8+4 plan boundary before any claim, random draw, controller input, frame, outcome, or fit. A coverage shortfall stops for reorientation rather than lowering the gate. |
| Next session | Perform one action-free inventory over authenticated repeatable, nonsealed, unconsumed Red captures; derive complete menus and observer bindings; freeze a path-free 8-train plus 4-disjoint-development plan if and only if the published coverage contract passes; then stop before outcome collection for exact-source publication and preflight review. |
| Next falsifier | The action-free inventory must reject a capture whose state/envelope/attestation join differs, any sealed, consumed, nonrepeatable, duplicate, synthetic, incomplete-menu, family-overlapping, or location-overlapping row, and any plan short of 8 train plus 4 development while proving claims, random draws, actions, frames, outcomes, and fits all remain zero. |
| Stop condition | Stop the inventory on identity leakage, an unverifiable capture attestation, any sealed or consumed root, synthetic counted executors, incomplete menus, train/development family or location overlap, or a coverage shortfall. Otherwise stop after the calibration fit and descriptive development report, before any powered benchmark, Crystal execution, promotion, or full replay. |

### Stop conditions

- Stop before real gameplay unless one action-free authenticated inventory proves complete menus, genuine executors, unique repeatable roots, eight train scenarios across four option kinds and three families, and four development scenarios disjoint from train by family and location.
