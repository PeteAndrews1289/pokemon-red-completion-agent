...
| Cheapest falsifier | A ROM-free concrete Red feasibility plan must show that fifteen preregistered setup slots can expose genuine complete three-or-more-kind semantic menus while preserving the published censor reserves, selected-kind probability, family and location separation, setup isolation, repeatable-root accounting, and private/public boundary. Any missing Red semantic executor must be named before gameplay rather than replaced with a route label. |
...
**2026-08-26-purpose-built-capture-contract-published** · status **active** · evidence [qualification](docs/evidence/living-dex-purpose-built-capture-contract-qualification-v1-2026-08-26.json)

| Check | Session conclusion |
| --- | --- |
| Product alignment | The project now has a shared cross-title definition of a useful lesson rather than another Red walkthrough artifact. It protects the eventual learner from setup arrows and forces Red-specific capture work to expose portable semantic choices whose realized consequences can train the same option-value layer later tested in Crystal. |
| Learning output | No new model example. The shared contract now requires at least ten train and five development setup slots, complete three-or-more-kind menus, exact at-least-98-percent prospective train kind coverage after two censors, selected-family and location survival, nonadaptive whole-slot accounting, and zero learner effects during setup. Counters remain development 15, verified 5, causal train 0, fits 4, comparisons 4, authority 0, and transfer 0. |
| Authority delta | None. Contract qualification used no ROM/private capture, behavior draw, learner claim, controller authority/action, frame advance, learner label, outcome, prediction, teacher query, fit, authority promotion, or transfer result. It authorizes neither a Red setup run nor model training. |
| Transfer result | Not run. The published curriculum schema is title-neutral, but only contract arithmetic has transferred: no Red capture adapter campaign and no Crystal adapter has been exercised. Crystal stays closed until authentic Red causal evidence and a powered Red benchmark justify a frozen-weight transfer test. |
| Blocker | The shared capture arithmetic and safety boundary are published, but no concrete Red ten-train plus five-development setup plan yet proves that the requested menus have genuine semantic executors. No durable Red setup runner is qualified. Authentic capture creation, selected-arm outcomes, fitting, and Crystal therefore remain closed. |
| Decision | Accept the published title-neutral capture contract as the replacement for legacy-bank archaeology. Next freeze a ROM-free concrete Red fifteen-slot feasibility plan and identify any missing reusable semantic skill before gameplay. Qualify the durable setup runner separately; do not combine plan design, protected setup execution, selected-arm outcomes, or fitting into one gate. |
| Next session | Design and freeze only the concrete prospective Red ten-train plus five-development setup plan and its semantic-skill feasibility report. Keep it ROM-free and path-free. Publish and reorient before implementing or running a durable setup campaign, opening a private capture, drawing an arm, collecting an outcome, fitting, opening sealed Red, executing Crystal, or replaying the game. |
| Next falsifier | A ROM-free Red feasibility planner must fail unless all fifteen slots have distinct preregistered purposes, complete planned semantic menus, sufficient family/location scopes under allowed censors, at least 98 percent exact selected-kind coverage, and genuine authenticated executor contracts. It must report missing semantic skills rather than silently substituting teacher routes, raw buttons, synthetic options, or title identities. |
| Stop condition | Stop before protected access if the concrete Red plan cannot satisfy the published fifteen-slot campaign, complete semantic-menu, selected-kind probability, censor-reserve, family/location, root, privacy, and setup-isolation contract with genuine executors. Stop rather than inventing routes or weakening coverage. A durable setup runner, outcomes, fitting, sealed Red, Crystal, promotion, and replay remain separate closed gates. |

### Stop conditions

- Stop before new gameplay capture until a published concrete Red fifteen-slot plan and separately qualified durable setup runner satisfy the purpose-built contract with genuine executors, complete 3+ kind menus, repeatable unique nonsealed roots, deterministic-setup isolation, exact selected-kind risk, censor reserves, and train/development family and location separation. V1, V2, and V3 may not retry.
