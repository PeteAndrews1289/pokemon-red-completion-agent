...
| Cheapest falsifier | Before another protected read, a ROM-free exhaustive successor must prove that one context-local restore, observation, enumeration, replay, or projection failure becomes a finite counted exclusion rather than aborting the bank, while global authentication/effect/integrity failures still stop and the unchanged 8+4 gate remains exact. |
...
**2026-08-26-red-authentic-inventory-v2-state-observation-failure** · status **active** · evidence [qualification](docs/evidence/red-living-dex-authentic-inventory-v2-failure-2026-08-26.json)

| Check | Session conclusion |
| --- | --- |
| Product alignment | V2 exposed the earliest authentic blocker instead of spending gameplay: one context-local state pipeline failure can hide the rest of the bank, while ten readable contexts lacked complete 3+ option menus. An exhaustive successor is the smallest bounded test of whether the existing Red classroom can support the title-neutral living-Pokedex learner or must be replaced with purpose-built captures. |
| Learning output | No new measured learner output. V2 authenticated 81 inputs, considered 12 contexts, read 11 states, authenticated 10 historical replays, excluded one consumed root and ten fewer-than-three-option contexts, and stopped at state_observation before projection or coverage. All protected effects and counter deltas were zero. Counters remain development 15, verified outcomes 5, causal train 0, model fits 4, unseen comparisons 4, authority 0, and transfer 0. |
| Authority delta | None. The V2 census made no model prediction, behavior draw, controller-authority attempt, game action, frame advance, outcome observation, fit, authority promotion, or transfer result. |
| Transfer result | Not run. The title-neutral feature, outcome, calibration, and model schema remains the intended transfer layer, while Red and Crystal retain separate observation adapters. Crystal stays closed until authentic Red causal evidence and a powered Red benchmark justify a frozen-weight transfer test. |
| Blocker | The sole V2 census stopped inside the twelfth considered context at state_observation after eleven state reads. One earlier root was consumed and ten successfully replayed contexts each had fewer than three mapped options. Scenario projection and exact 8+4 coverage never ran, so the remaining bank and curriculum sufficiency remain unknown. |
| Decision | Retire V2 without retry. Qualify a ROM-free exhaustive successor that splits restore, observation, enumeration, replay, and projection; counts only preregistered context-local failures as exclusions and continues; reconciles all 81 contexts; and still fails globally on source/input/runtime/namespace/effect/integrity/publication errors. Publish and reorient before any new inventory identity may read protected inputs. |
| Next session | Implement and qualify only the exhaustive path-free successor boundary. Publish through exact-source CI, then reorient before deciding whether one new identity merits one authenticated census. Do not retry V1/V2, open a ROM/private capture during qualification, collect an outcome, fit, run Crystal, open sealed Red, or replay the game. |
| Next falsifier | ROM-free adversarial tests must prove exhaustive context accounting, distinguish restore/observe/enumerate/replay/project exclusions, prevent any context-local exception from aborting later synthetic contexts, kill misclassified fatal errors and counter mismatches, reject identity/path leakage, and retain every nonzero protected effect. Any survivor stops before protected access. |
| Stop condition | Stop the successor before protected access if any context-local category is ambiguous, a synthetic local failure can abort the remaining bank, a fatal global failure can be downgraded, an aggregate counter can disagree, a private identity/path can escape, or a protected effect can be falsely zero. V1/V2 never retry; outcome collection, sealed Red, Crystal, promotion, and full replay remain closed. |

### Stop conditions

- Stop before real gameplay until a published exhaustive action-free successor accounts for the complete authenticated bank and proves complete menus, genuine executors, unique repeatable roots, eight train scenarios across four option kinds and three families, and four development scenarios disjoint from train by family and location. V1 and V2 may not retry, and no successor may read protected inputs before ROM-free qualification and reorientation.
