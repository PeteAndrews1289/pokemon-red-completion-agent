...
| Cheapest falsifier | One new-identity V2 action-free authenticated census must either freeze the unchanged 8-train plus 4-development curriculum or stop once with a finite exact reason and mathematically reconciled aggregate counters while every protected effect stays at zero. |
...
**2026-08-26-red-authentic-inventory-v2-diagnostics-published** · status **active** · evidence [qualification](docs/evidence/red-living-dex-authentic-inventory-v2-diagnostic-qualification-2026-08-26.json)

| Check | Session conclusion |
| --- | --- |
| Product alignment | V2 removes the exact blindness that made another authenticated read wasteful. One bounded census now either proves the real 8+4 classroom exists or identifies the smallest genuine data/curriculum blocker without changing the gate, which is the shortest honest bridge to the first causal living-Pokedex model. |
| Learning output | No new measured learner output. Published main cca0eb8d passed exact-main CI with 4,910 tests. Ninety-one focused V2 tests distinguish every private stage and exact coverage reason, kill 25 receipt mutations, and retain any nonzero protected effect. Qualification opened no ROM or private capture. Counters remain development 15, verified outcomes 5, causal train 0, model fits 4, unseen comparisons 4, authority 0, and transfer 0. |
| Authority delta | None. V2 qualification used no private input, model prediction, behavior draw, game action, outcome observation, fit, authority promotion, or transfer result. |
...
| Blocker | The V2 diagnostic and one-shot runner are published, but the authentic capture bank has not been reopened under that boundary. Whether it can satisfy the unchanged 8+4 option-kind, family, location, provenance, and disjointness gate remains unknown. |
| Decision | Permit exactly one new-identity V2 action-free authenticated census from published main cca0eb8d under exact-main green CI 32942245005. It may freeze a private plan only if the unchanged gate passes; otherwise its finite reason and aggregate counts are the result. V1 and V2 never retry. |
| Next session | Invoke the published V2 runner exactly once against the authenticated repeatable, nonsealed, unconsumed Red capture bank. Publish only its path-free terminal and reorient immediately. Do not retry V1 or V2, collect an outcome, fit, run Crystal, open sealed Red, or replay the game. |
| Next falsifier | The sole V2 census must account for every authenticated context through fixed exclusions, namespaces, state reads, historical replays, projections, zero-effect checks, coverage, encoding, integrity, and publication. It either publishes the exact 8+4 plan or returns one finite failure plus its safe coverage counts; any ambiguity, counter mismatch, leak, or nonzero protected effect stops. |
| Stop condition | Stop after the first V2 terminal, whether it is a frozen plan or a finite exact failure. Never retry either inventory identity or weaken the 8+4 gate. Stop immediately on ambiguity, identity/path leakage, counter mismatch, or any controller authority, action, frame, draw, claim, outcome, prediction, teacher query, or fit. Outcome collection, sealed Red, Crystal, promotion, and full replay remain closed. |

### Stop conditions

- Stop before real gameplay unless the sole published V2 action-free authenticated census proves complete menus, genuine executors, unique repeatable roots, eight train scenarios across four option kinds and three families, and four development scenarios disjoint from train by family and location. V1 and V2 may not retry.
