...
| Cheapest falsifier | A ROM-free routed semantic binding must fail unless it authenticates and verifies the declared route terminal, freshly reobserves the title state, binds the unchanged destination kind through an implemented provider contract, executes that destination exactly once, and requires both route and destination success under one reconciled budget. A route relabelled as the semantic destination is a failure. |
...
**2026-08-26-red-prospective-capture-plan-routed-goal-blocker** · status **active** · evidence [qualification](docs/evidence/red-living-dex-prospective-capture-plan-v1-qualification-2026-08-26.json)

| Check | Session conclusion |
| --- | --- |
| Product alignment | The project now has a statistically adequate Red lesson schedule and an exact reusable blocker. Route-then-semantic-skill composition is shared infrastructure that later titles also need, while preserving acquisition, evolution, storage, resource, party, and story choices as model-facing semantics instead of Red arrow sequences. |
| Learning output | No new model example. The exact 10+5 Red schedule uses fifteen complete three-kind menus and reaches 2144/2187, or 98.0338 percent, selected-kind coverage after any two train censors. Seven kinds have implemented Red provider contracts; one slot is locally composable and fourteen need routing. Counters remain development 15, verified 5, causal train 0, fits 4, comparisons 4, authority 0, and transfer 0. |
| Authority delta | None. Plan qualification used no ROM/private capture, behavior draw, learner claim, controller authority/action, frame advance, learner label, outcome, prediction, teacher query, fit, authority promotion, or transfer result. It authorizes neither a Red setup run nor model training. |
| Transfer result | Not run. The schedule and proposed composition contract are title-neutral, but no runtime Red capture or Crystal adapter has been exercised. Crystal stays closed until authentic Red causal evidence and a powered Red benchmark justify a frozen-weight transfer test. |
| Blocker | The exact Red fifteen-slot schedule is valid, but only one menu is co-located at existing skill boundaries. Fourteen require a generic routed-semantic-goal composition seam that does not exist. Trade is a separate later full-living-Pokedex gap. No durable setup runner is qualified, so authentic captures, outcomes, fitting, and Crystal remain closed. |
| Decision | Accept the published Red plan as a valid curriculum design and its execution-blocked verdict as the intended falsifier. Implement only the reusable route-then-semantic-goal composition seam next. Keep the destination kind and verifier authoritative; do not turn route traversal into a learner-facing label. Publish and reorient before a durable setup runner or private capture campaign. |
| Next session | Implement and adversarially qualify only the title-neutral routed-semantic-goal composition contract plus the minimal Red adapter needed to bind existing route and destination providers. Keep it ROM-free. Publish and reorient before building a durable fifteen-slot setup runner, opening private captures, drawing an arm, collecting an outcome, fitting, opening sealed Red, executing Crystal, or replaying the game. |
| Next falsifier | The routed seam must reject a route-terminal mismatch, destination-kind drift, a missing fresh reobservation, an unavailable destination provider, a second destination execution, either verifier failure, nonreconciled action/frame totals, and any report that lets the route masquerade as the semantic outcome. Passing synthetic tests must not imply a runtime Red binding. |
| Stop condition | Stop if the generic composition cannot preserve the declared destination kind, independently verify both route and destination, reconcile one bounded execution, and prevent route details from entering learner labels. Do not solve a failing generic test with a fixed Red route. A durable runner, private setup, outcomes, fitting, sealed Red, Crystal, promotion, and replay remain separate closed gates. |

### Stop conditions

- Stop before new gameplay capture until the published Red fifteen-slot plan is paired with a qualified routed-semantic-goal seam and a separately qualified durable setup runner. The seam must preserve destination semantics, independently verify route and destination, reconcile its budget, and expose no route label or teacher sequence. V1, V2, and V3 may not retry.
