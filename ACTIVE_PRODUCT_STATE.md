...
| Cheapest falsifier | Under the exact clean published Red execution identity, the action-free freezer must reproduce fifteen slots, forty-five real provider offers, thirty-three mechanics-derived semantic families, and ten physical origins from the same still-unconsumed roots while every protected-effect counter remains zero. Another cartridge, a consumed or cross-joined root, incoherent prerequisites, unavailable provider, or incomplete capacity must fail before controller authority. |
...
**2026-08-26-authentic-provider-curriculum-qualified-plan-freeze-next** · status **active** · evidence [qualification](docs/evidence/red-living-dex-authentic-provider-curriculum-local-qualification-2026-08-26.json)

| Check | Session conclusion |
| --- | --- |
| Product alignment | The project now has a real, diverse Red lesson curriculum built from reusable mechanics rather than a walkthrough or synthetic family names. Freezing it under the exact published identity is the shortest remaining bridge to repeatable same-root captures, randomized causal outcomes, a train-only fit, and later frozen-weight Crystal transfer. |
| Learning output | No new model example. The action-free curriculum qualification produced fifteen authentic recipe roots and forty-five genuine prospective offers but intentionally produced no setup capture, selected-arm outcome, or model fit. Counters remain development 15, verified 5, causal train 0, fits 4, comparisons 4, authority 0, and transfer 0. |
| Authority delta | None. The authentic preflight read private Red roots and cartridge structure but moved no controller or frame, executed no provider, made no behavior draw or prediction, queried no teacher, emitted no label or outcome, fit no model, claimed no root, and promoted no authority. |
...
| Blocker | The genuine provider curriculum now exists and the selected authentic roots pass the complete 15/45/33/10 action-free preflight. The remaining blocker is a canonical plan artifact bound to the exact clean published Red source and execution identity. Setup execution remains closed. Trade, version, event, cross-save, and living-storage consolidation remain later mission gaps. |
| Decision | Accept implementation checkpoint 6063cb41 and the path-free authentic action-free qualification. Publish the exact implementation and generated registries, bind the clean Red execution identity, freeze the same complete plan once with protected effects zero, publish only aggregates, and reorient before any setup execution. |
| Next session | Publish the authentic provider implementation under green exact-source CI, derive the clean Red execution identity, freeze one canonical action-free 15/45/33/10 private plan, publish only its path-free aggregate, and reorient before invoking the setup campaign. |
| Next falsifier | The publish-bound freezer must reproduce the complete 15/45/33/10 plan against the exact supported Red ROM and same available physical roots with every protected-effect delta zero. It must reject wrong ROM identity, dirty source, consumed or cross-joined roots, incoherent party/collection facts, unavailable provider prerequisites, and incomplete capacity. |
| Stop condition | Stop before controller input during the exact plan freeze; on another cartridge, dirty or unpublished source, consumed/cross-joined roots, incoherent prerequisites, unavailable providers, any protected effect, or incomplete 15/45/33/10 capacity. Setup execution, outcomes, fitting, sealed Red, Crystal, promotion, and replay remain closed. |
