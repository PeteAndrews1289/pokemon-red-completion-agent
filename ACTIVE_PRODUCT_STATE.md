...
| Cheapest falsifier | Before any root claim, the private recipe plan must cover all fifteen frozen slots, forty-five genuine provider recipes, ten one-to-one physical origin-map scopes, and cross-scope-disjoint expected transformation families. ROM-free fork validation must hash the exact source bytes, reproduce the complete origin observation before every arm, derive the actual matching family from a fresh real offer after that arm's route, execute no provider, and permanently reject any mismatch or learner effect. |
...
**2026-08-26-same-root-recipe-qualified-provider-family-next** · status **active** · evidence [qualification](docs/evidence/red-living-dex-same-root-setup-recipe-v1-local-qualification-2026-08-26.json)

| Check | Session conclusion |
| --- | --- |
| Product alignment | The same-root contract turns purpose-built Red setup into causal multi-option lessons: every arm begins from the identical state, transport remains private, and actual provider families are frozen and verified without becoming policy identity. This is the minimum honest bridge to eight causal living-Pokedex train outcomes and a title-neutral value model that can later be tested in Crystal. |
| Learning output | No new model example. The same-root contract has 26 direct and 156 related ROM-free tests. It freezes exact source bytes, per-arm restore equality, genuine post-route offers, prospective family separation, and durable no-retry terminals, but it created no authentic Red capture or outcome. Counters remain development 15, verified 5, causal train 0, fits 4, comparisons 4, authority 0, and transfer 0. |
| Authority delta | None. Same-root recipe qualification used only synthetic ROM-free runtimes. It read no authentic private capture or ROM, moved no controller or frame, made no behavior draw or model prediction, invoked no candidate provider or teacher, emitted no label or outcome, fit no model, and promoted no authority. |
...
| Blocker | No authentic fifteen-recipe plan exists. The historical pre-observed catalog is insufficient and cannot be cross-joined honestly. Current Red evolution exposes only the Diglett-to-Dugtrio family and balanced development exposes one generic one-level family, so repeated evolution/development kinds cannot yet satisfy the frozen cross-scope family separation. Trade remains a separate later full-living-Pokedex gap. |
| Decision | Retire the historical cross-join path, publish the same-root recipe and durable campaign contract, then inventory actual prospective provider families action-free and implement only the smallest reusable target-specific evolution/development offers needed to freeze a genuine fifteen-recipe plan. Reorient before private setup execution. |
| Next session | Publish the same-root contract, inventory genuine provider-family identities without controller input, implement reusable target-specific evolution/development offers for the measured deficits, and freeze a complete fifteen-recipe plan. Stop and reorient before any private setup execution, behavior draw, outcome, fit, sealed Red, Crystal, promotion, or replay. |
| Next falsifier | An action-free provider-family inventory and complete recipe-plan build must either produce fifteen slots, forty-five actual provider families, and ten physical origins with no cross-scope family/location overlap, or stop with the exact family deficit. Slot, root, profile, or route identity may not be used to manufacture a family. |
| Stop condition | Stop if genuine family breadth requires cross-joining old captures, encoding a slot/root/profile identity as a family, inventing a synthetic provider, hard-coding a teacher route, weakening family/location separation, or opening private setup before publication and reorientation. Outcomes, fitting, sealed Red, Crystal, promotion, and replay remain closed. |

### Stop conditions

- Stop before new gameplay capture until the same-root recipe contract is published and one action-free plan build freezes all fifteen slots, forty-five genuine provider recipes, ten physical origin maps, and prospectively disjoint actual family identities. Do not cross-join historical captures or use slot/root/profile identity as a family. A claimed setup may never retry.
