...
**2026-08-26-source-adapter-locally-qualified-publication-next** · status **active** · evidence [qualification](docs/evidence/red-living-dex-setup-source-adapter-v1-local-qualification-2026-08-26.json)

| Check | Session conclusion |
| --- | --- |
| Product alignment | The project now has the statistically adequate Red lesson schedule, reusable route-to-semantic composition, durable no-retry setup runner, action-free whole-plan materializer, and a locally qualified concrete Red source adapter. Exact-source publication and one authentic zero-action plan freeze are the remaining bridge to decision-rich setup captures and the first eight causal train outcomes, while learner-facing choices remain title-neutral for later Crystal transfer. |
| Learning output | No new model example. The adapter passed 24 direct, 177 combined related-regression, and 5,111 full-suite tests. It derives the exact fifteen-slot, forty-five-provider, forty-two-route shape only from typed synthetic ROM-free witnesses and never invokes an executable or verifier; no authentic private plan was used. Counters remain development 15, verified 5, causal train 0, fits 4, comparisons 4, authority 0, and transfer 0. |
| Authority delta | None. The concrete source-adapter qualification used only synthetic ROM-free witnesses and no private capture, ROM, controller action, frame advance, behavior draw, learner claim, label, outcome, prediction, teacher query, fit, authority promotion, or transfer result. It authorizes neither actionful Red setup nor model training. |
| Transfer result | Not run. The concrete Red adapter keeps title-private routes, observations, providers, and catalog paths behind the shared action-free binding shape, but no actual Red plan or Crystal adapter has been executed. Crystal stays closed until authentic Red causal evidence and a powered Red benchmark justify a frozen-weight transfer test. |
| Blocker | The concrete private Red source adapter and owner-private action-free freezer are locally qualified but not yet published under exact-source green CI. No authentic fifteen-slot catalog has been read, authenticated, or sealed. Trade remains a separate later full-living-Pokedex gap; actionful setup, outcomes, fitting, sealed Red, and Crystal remain closed. |
| Decision | Accept the locally green concrete adapter and close its implementation work. Publish the exact source under green CI, then record one separate action-free private-input gate and run the freezer once. Reorient on its aggregate terminal before invoking the durable setup runner. |
| Next session | Publish the exact concrete source adapter under green source-bound CI. Then, under a separate action-free-only gate, authenticate approved private witnesses and run the fifteen-slot freezer once. Publish only its aggregate terminal and stop before setup execution, behavior draw, outcome collection, fitting, sealed Red, Crystal, promotion, or replay. |
| Next falsifier | Exact-source CI must remain green, then one separately recorded action-free private catalog freeze must authenticate every approved input and all 15/45/42 joins with zero protected-effect deltas. Any missing, reordered, aliased, changed, synthetic, unavailable, teacher-derived, weakly protected, actionful, or identity-leaking input is a terminal failure. |
| Stop condition | Stop if publication is not exact-source green or if the private plan freeze needs controller input, frame advance, a teacher route, fixed directions, a synthetic menu arm, an identity-bearing public feature, a changed protected input, a changed frozen slot, or a retry. Do not invoke the setup runner in this gate. Outcomes, fitting, sealed Red, Crystal, promotion, and replay remain separate closed gates. |
