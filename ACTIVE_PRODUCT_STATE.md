...
| --- | --- |
| Reusable capability | Create repeatable same-root Red living-Pokedex decisions as complete variable-size title-neutral menus: freeze genuine provider families and physical origins before input, restore the exact captured state before every candidate, derive each offer only after its own semantic route, then later randomize and execute one authenticated option and learn from its independently observed outcome. |
...
| Cheapest falsifier | Before any root claim, a malicious ROM-free adapter that ignores supplied state bytes, fabricates a route report, fabricates a provider family or offer, spends protected authority, mutates a nested schema, or crosses a crash cutpoint must be rejected or recovered without retry. Only after that gate may a plan attempt fifteen slots, forty-five real provider offers, at least thirty-three distinct semantic family identities, and ten one-to-one physical origin-map scopes. |
...
**2026-08-26-same-root-adversarial-audit-repair-next** · status **active** · evidence [qualification](docs/evidence/red-living-dex-same-root-setup-recipe-v1-adversarial-audit-2026-08-26.json)

| Check | Session conclusion |
| --- | --- |
| Product alignment | Trustworthy same-root lessons remain the shortest bridge to authentic living-Pokedex option-value training, but the current synthetic contract does not yet prove that each arm actually begins from the same emulator state or that route/provider evidence is independent. Repairing that boundary prevents fabricated Red lessons from becoming model targets and keeps the later family schema transferable to Crystal. |
| Learning output | No new model example. Three independent read-only audits and owner reproductions changed no counter. The previous 26 focused, 156 related, and 5,137 full-suite tests remain useful regression evidence, but synthetic success did not prove exact emulator restore, authentic route/provider execution, comprehensive zero effects, or genuine causal families. Counters remain development 15, verified 5, causal train 0, fits 4, comparisons 4, authority 0, and transfer 0. |
| Authority delta | None. The audit and owner reproductions were ROM-free and read-only. They opened no private capture or ROM, moved no controller or frame, made no behavior draw or model prediction, invoked no candidate provider or teacher, emitted no label or outcome, fit no model, and promoted no authority. |
| Transfer result | Not run. The same-root menu, outcome, and later model interfaces remain title-neutral while Red routes, profiles, observations, and providers stay private. No Crystal fixture or game executed; Crystal remains the first frozen-weight test only after authentic Red causal evidence and a powered Red benchmark. |
| Blocker | The same-root scaffolding is not yet a trustworthy causal boundary. One opaque runtime may ignore state bytes and self-attest restore, route, family, and provider truth; only actions and frames are measured; execution identity is incomplete; proof joins and schema restores are weak; no-retry is not account-wide; and crash cutpoints are incomplete. No authentic plan exists. Trade, version, event, cross-save, and living-storage gates remain later mission gaps. |
| Decision | Hold PR 78 as non-publishable infrastructure and repair the causal trust boundary before widening provider coverage. Preserve the schema and durable claims, but add sealed execution identity, comprehensive effect measurement, restore acknowledgement, authenticated route reports, registry-constructed offers, typed semantic families, complete proof joins, account-wide root consumption, and crash-restart tests. Reorient before provider inventory or private setup execution. |
| Next session | Repair the same-root trust boundary: proof joins and schema checks, closed public failure codes, sealed Red execution identity, comprehensive protected-effect checkpoints, fork-isolated arm runtimes, restore acknowledgement, authenticated route reports, registry-constructed action-free offers, typed semantic families, account-wide root consumption, and failpoint restart tests. If all gates pass, reorient before provider expansion or private setup. |
| Next falsifier | A malicious ROM-free adapter must fail if it ignores supplied state bytes, fabricates route or provider evidence, invents a family from slot/root/profile/route identity, spends any protected authority, mutates nested schema identity, swaps fork proofs, retries a consumed root through another store, or crashes between durable transitions. Only after those falsifiers pass may the project inventory forty-five real offers with a thirty-three-family minimum. |
| Stop condition | Stop before provider expansion or gameplay if restore acknowledgement, authenticated route reports, sealed provider construction, complete effect metering, and account-wide root consumption cannot reuse existing seams without a second controller or title-specific monolith. Also stop on cross-joined captures, identity-derived families, synthetic counted providers, teacher routes, or weakened family/location separation. Outcomes, fitting, sealed Red, Crystal, promotion, and replay remain closed. |

### Stop conditions

- Stop before gameplay until the same-root boundary authenticates restore, route execution, provider construction, family identity, all protected effects, proof joins, account-wide root consumption, and crash recovery. Then require fifteen slots, forty-five real offers, at least thirty-three semantic families, and ten physical origin maps. Do not cross-join captures or derive families from slot/root/profile/route identity. A claimed physical root may never retry through another store or plan.
...
- **Claude:** Review statistics, leakage, experimental power, and semantic overclaims at milestone decisions; remain read-only and do not duplicate Codex implementation work.
- **Antigravity:** Review architecture, claim boundaries, and cross-game transfer at milestone decisions; remain read-only and do not block ordinary development while no review is pending.
