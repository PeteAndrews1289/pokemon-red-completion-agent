...
class SameDestinationRoutePair:
    """A shortest route and a deterministic, strictly costlier detour."""

    shortest: RoutePlan
    detour: RoutePlan
    excluded_step_ordinal: int
    excluded_map: int
    excluded_at: tuple[int, int]

    def __post_init__(self) -> None:
        if not isinstance(self.shortest, RoutePlan) or not isinstance(
            self.detour, RoutePlan
        ):
            raise TypeError("same-destination routes must be RoutePlan values")
        if type(self.excluded_step_ordinal) is not int or not (  # noqa: E721
            1 <= self.excluded_step_ordinal <= len(self.shortest.steps)
        ):
            raise ValueError("excluded route step ordinal is invalid")
        excluded = self.shortest.steps[self.excluded_step_ordinal - 1]
        if (
            not excluded.can_discover_blocker
            or excluded.source_map != self.excluded_map
            or excluded.expected_at != self.excluded_at
        ):
            raise ValueError("excluded route coordinate differs from the shortest plan")
        if (
            self.shortest.macro_path.maps[0],
            self.shortest.start_at,
            self.shortest.start_mode,
        ) != (
            self.detour.macro_path.maps[0],
            self.detour.start_at,
            self.detour.start_mode,
        ):
            raise ValueError("same-destination routes do not share one start")
        if (
            self.shortest.terminal_map,
            self.shortest.terminal_at,
            self.shortest.terminal_mode,
        ) != (
            self.detour.terminal_map,
            self.detour.terminal_at,
            self.detour.terminal_mode,
        ):
            raise ValueError("same-destination routes do not share one terminal")
        if self.detour.cost <= self.shortest.cost:
            raise ValueError("same-destination detour is not strictly costlier")
        if self.detour.actions == self.shortest.actions:
            raise ValueError("same-destination routes are not behaviorally distinct")
...
class StrategicScenarioRouteWorld:
    ...
    def plan_same_destination_pair(
        ...
        *,
        ...
        """Plan a shortest route and the cheapest strictly longer valid detour.

        The detour is generated by excluding one walk coordinate from the
        shortest plan during planning.  The exclusion is only a construction
        constraint: both returned plans remain executable candidates in the
        unchanged world, share an exact terminal, and are chosen before any
        candidate outcome is observed.
        """

        shortest = self.plan_to_map(start, goal_map, goal_at=goal_at)
        occupied: dict[int, frozenset[tuple[int, int]]] = {
            start.map_id: start.occupied
        }
        alternatives: list[SameDestinationRoutePair] = []
        for ordinal, step in enumerate(shortest.steps, start=1):
            if not step.can_discover_blocker:
                continue
            blocked = dict(occupied)
            blocked[step.source_map] = blocked.get(
                step.source_map,
                frozenset(),
            ).union({step.expected_at})
            try:
                detour = self._plan_candidate(
                    start,
                    goal_map,
                    goal_at=goal_at,
                    blocked=blocked,
                ...
                continue
            if (
                detour.cost <= shortest.cost
                or detour.actions == shortest.actions
                or (
                    detour.terminal_map,
                    detour.terminal_at,
                    detour.terminal_mode,
                )
                != (
                    shortest.terminal_map,
                    shortest.terminal_at,
                    shortest.terminal_mode,
                )
            ):
                continue
            alternatives.append(
                SameDestinationRoutePair(
                    shortest=shortest,
                    detour=detour,
                    excluded_step_ordinal=ordinal,
                    excluded_map=step.source_map,
                    excluded_at=step.expected_at,
                )
            )
        if not alternatives:
            raise RoutePlanningError(
                "no strictly costlier same-destination route candidate exists"
            )
        return min(
            alternatives,
            key=lambda pair: (
