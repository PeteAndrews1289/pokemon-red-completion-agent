...
def run_bruno_chapter(
    ...
    *,
    ...
    try:
        for party_index, item in _post_bruno_recovery_plan(
            _party_hp(emulator),
            _party_max_hp(emulator),
            _party_status(emulator),
        ):
            _use_post_bruno_item(actions, reader, emulator, party_index, item)
...
def _post_bruno_recovery_plan(
    party_hp: tuple[int, ...],
    party_max_hp: tuple[int, ...],
    party_status: tuple[int, ...],
) -> tuple[tuple[int, ItemId], ...]:
    """Plan field recovery for every affected member, not just the party lead."""
    if not (len(party_hp) == len(party_max_hp) == len(party_status)):
        raise BrunoChapterError("Post-Bruno party recovery state is incomplete.")
    ...
    for index, (hp, max_hp, status) in enumerate(
        ...
        if not 0 < hp <= max_hp:
            raise BrunoChapterError(
                f"Post-Bruno party member {index + 1} has invalid HP {hp}/{max_hp}."
            )
...
def _use_post_bruno_item(
    ...
    """Use and verify one field recovery item on its planned party member."""
    ...
    if (
        ...
        raise BrunoChapterError(
            f"Post-Bruno {item.name} target {party_index + 1} lacks valid evidence."
        )
    ...
    for _ in range(24):
        ...
        hp_recovered = current_hp[party_index] == expected_hp
        status_cured = current_status[party_index] == 0
        if item_consumed and hp_recovered and status_cured:
            _close_menus(actions, reader, DEFAULT_LAVENDER_TIMING)
            return
        _pulse(actions, MacroActionKind.CONFIRM)
    raise BrunoChapterError(
        f"Post-Bruno {item.name} did not recover party member {party_index + 1}."
    )
